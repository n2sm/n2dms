...
public class PDFUtils {
    ...
    public static void fillForm(final InputStream input,
            final Map<String, Object> values, final OutputStream output)
            throws FileNotFoundException, DocumentException, TemplateException,
            IOException {
        log.debug("fillForm({}, {}, {})",
                new Object[] { input, values, output });
        ...
        if (form != null) {
            for (final Iterator it = form.getFields().iterator(); it.hasNext();) {
                final PRAcroForm.FieldInformation field = (PRAcroForm.FieldInformation) it
                        .next();
                final String fieldValue = fields.getField(field.getName());
                log.debug("Field: {}, Value: '{}'", field.getName(), fieldValue);

                if (fieldValue != null && !fieldValue.isEmpty()) {
                    // if (values.containsKey(field.getName())) {
                    final String result = TemplateUtils.replace(
                            "PDF_FILL_FORM", fieldValue, values);
                    log.debug("Field '{}' set to '{}' (by expression)",
                    ...
                    if (value != null) {
                        log.debug("Field '{}' set to '{}' (by field name)",
                                field.getName(), value);
                        ...
                        log.warn(
                                "Field '{}' (value ignored because not included in map)",
                                field.getName());
                    }
                }
            }
     ...
     */
    @SuppressWarnings("rawtypes")
    public static List<String> listFormFields(final String input)
