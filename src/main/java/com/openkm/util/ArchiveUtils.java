...
public class ArchiveUtils {
     ...
     */
    public static void createZip(final File path, final OutputStream os)
            throws IOException {
        log.debug("createZip({}, {})", new Object[] { path, os });

        if (path.exists() && path.canRead()) {
            final ZipArchiveOutputStream zaos = new ZipArchiveOutputStream(os);
            ...
            log.debug("FILE {}", path);
            final ZipArchiveEntry zae = new ZipArchiveEntry(path.getName());
     ...
     */
    public static void createZip(final File path, final String root,
        ...
        if (path.exists() && path.canRead()) {
            ...
            final ZipArchiveEntry zae = new ZipArchiveEntry(root + "/");
            zaos.putArchiveEntry(zae);
            zaos.closeArchiveEntry();
     ...
     */
    private static void createZipHelper(final File fs,
            final ZipArchiveOutputStream zaos, final String zePath)
            throws IOException {
        log.debug("createZipHelper({}, {}, {})", new Object[] { fs, zaos,
                zePath });
        ...
        for (final File file : files) {
            if (file.isDirectory()) {
                ...
                final ZipArchiveEntry zae = new ZipArchiveEntry(zePath + "/"
                        + file.getName() + "/");
                zaos.putArchiveEntry(zae);
                zaos.closeArchiveEntry();

                createZipHelper(file, zaos, zePath + "/" + file.getName());
            } else {
                log.debug("FILE {}", file);
                final ZipArchiveEntry zae = new ZipArchiveEntry(zePath + "/"
                        + file.getName());
     ...
     */
    public static void createJar(final File path, final String root,
            final OutputStream os) throws IOException {
        log.debug("createJar({}, {}, {})", new Object[] { path, root, os });

        if (path.exists() && path.canRead()) {
            final JarArchiveOutputStream jaos = new JarArchiveOutputStream(os);
            ...
            final JarArchiveEntry jae = new JarArchiveEntry(root + "/");
     ...
     */
    private static void createJarHelper(final File fs,
            final JarArchiveOutputStream jaos, final String zePath)
            throws IOException {
        log.debug("createJarHelper({}, {}, {})", new Object[] { fs, jaos,
                zePath });
        final File[] files = fs.listFiles();
        ...
        for (final File file : files) {
            if (file.isDirectory()) {
                log.debug("DIRECTORY {}", file);
                final JarArchiveEntry jae = new JarArchiveEntry(zePath + "/"
                        + file.getName() + "/");
                jaos.putArchiveEntry(jae);
                jaos.closeArchiveEntry();
     ...
     * Read file from ZIP
     */
    public static byte[] readFileFromZip(final ZipInputStream zis,
     ...
     * Read file from ZIP
     */
    public static InputStream getInputStreamFromZip(final ZipInputStream zis,
            final String filename) throws IOException {
        ZipEntry zi = null;
        InputStream is = null;
