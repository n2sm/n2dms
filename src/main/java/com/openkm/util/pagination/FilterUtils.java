...
public class FilterUtils {
    ...
    public static FilterResult filter(GWTWorkspace workspace, List<Object> col, int order, String selectedRowId,
            Map<String, GWTFilter> mapFilter) throws PrincipalAdapterException, IOException, ParseException, NoSuchGroupException,
            ...
            for (String key : mapFilter.keySet()) {
                ...
                case GWTPaginated.COL_SIZE:
                    if (obj instanceof NodeFolder) {
                        ...
                        if (mapFilter.get(key).getSizeValue1() >= 0 && ((NodeMail) obj).getSize() < mapFilter.get(key).getSizeValue1()) {
                            add = false; // Should not be added to final list
                        }

                        if (mapFilter.get(key).getSizeValue2() >= 0 && ((NodeMail) obj).getSize() > mapFilter.get(key).getSizeValue2()) {
                            add = false; // Should not be added to final list
                        }
                    }
                    break;
                ...
                case GWTPaginated.COL_DATE:
                    if (obj instanceof NodeFolder) {
                        ...
                        if (mapFilter.get(key).getFrom() != null
                                && ((NodeDocument) obj).getLastModified().getTime().compareTo(mapFilter.get(key).getFrom()) <= 0) {
                            add = false;
                        }

                        if (mapFilter.get(key).getTo() != null
                                && ((NodeDocument) obj).getLastModified().getTime().compareTo(mapFilter.get(key).getTo()) >= 0) {
                            add = false;
                        }
                    } else if (obj instanceof NodeMail) {
                        if (mapFilter.get(key).getFrom() != null
                ...
                case GWTPaginated.COL_AUTHOR:
                    if (obj instanceof NodeFolder) {
                        ...
                        if (!OKMAuth.getInstance().getName(null, nDocVer.getAuthor()).toLowerCase()
                                .contains(mapFilter.get(key).getFilterValue1())) {
                            add = false; // Should not be added to final list
                        }
                    } else if (obj instanceof NodeMail) {
     ...
     * filter
     */
    public static void filter(GWTWorkspace workspace, List<?> col, Map<String, GWTFilter> mapFilter) throws PrincipalAdapterException,
            ...
            for (String key : mapFilter.keySet()) {
                ...
                case GWTPaginated.COL_NAME:
                    if (obj instanceof GWTFolder) {
                        if (!((GWTFolder) obj).getName().toLowerCase().contains(mapFilter.get(key).getFilterValue1())) {
                            add = false; // Should not be added to final list
                        }
                    } else if (obj instanceof GWTDocument) {
                        if (!((GWTDocument) obj).getName().toLowerCase().contains(mapFilter.get(key).getFilterValue1())) {
                            add = false; // Should not be added to final list
                        }
                    } else if (obj instanceof GWTMail) {
                        if (!((GWTMail) obj).getSubject().toLowerCase().contains(mapFilter.get(key).getFilterValue1())) {
                            add = false; // Should not be added to final list
                        }
                    }
                    break;
                ...
                case GWTPaginated.COL_SIZE:
                    if (obj instanceof GWTFolder) {
                        ...
                        if (mapFilter.get(key).getSizeValue1() >= 0
                                && ((GWTDocument) obj).getActualVersion().getSize() < mapFilter.get(key).getSizeValue1()) {
                            add = false; // Should not be added to final list
                        }
                        if (mapFilter.get(key).getSizeValue2() >= 0
                                && ((GWTDocument) obj).getActualVersion().getSize() > mapFilter.get(key).getSizeValue2()) {
                            add = false; // Should not be added to final list
                        }
                    } else if (obj instanceof GWTMail) {
                        if (mapFilter.get(key).getSizeValue1() >= 0 && ((GWTMail) obj).getSize() < mapFilter.get(key).getSizeValue1()) {
                            add = false; // Should not be added to final list
                        }
                        if (mapFilter.get(key).getSizeValue2() >= 0 && ((GWTMail) obj).getSize() > mapFilter.get(key).getSizeValue2()) {
                            add = false; // Should not be added to final list
                        }
                    }
                    break;
                ...
                case GWTPaginated.COL_DATE:
                    if (obj instanceof GWTFolder) {
                        ...
                        if (mapFilter.get(key).getTo() != null && ((GWTFolder) obj).getCreated().compareTo(mapFilter.get(key).getTo()) >= 0) {
                            add = false;
                        }
                    } else if (obj instanceof GWTDocument) {
                        if (mapFilter.get(key).getFrom() != null
                                && ((GWTDocument) obj).getActualVersion().getCreated().compareTo(mapFilter.get(key).getFrom()) <= 0) {
                            add = false;
                        }
                        if (mapFilter.get(key).getTo() != null
                                && ((GWTDocument) obj).getActualVersion().getCreated().compareTo(mapFilter.get(key).getTo()) >= 0) {
                            add = false;
                        }
                    } else if (obj instanceof GWTMail) {
                        if (mapFilter.get(key).getFrom() != null
                        ...
                        if (mapFilter.get(key).getTo() != null && ((GWTMail) obj).getCreated().compareTo(mapFilter.get(key).getTo()) >= 0) {
                            add = false;
                        }
                    }
                    break;
                ...
                case GWTPaginated.COL_AUTHOR:
                    if (obj instanceof GWTFolder) {
                        ...
                        if (!OKMAuth.getInstance().getName(null, ((GWTDocument) obj).getActualVersion().getAuthor()).toLowerCase()
                                .contains(mapFilter.get(key).getFilterValue1())) {
                            add = false; // Should not be added to final list
                        }
                    } else if (obj instanceof GWTMail) {
                ...
                case GWTPaginated.COL_VERSION:
                    if (obj instanceof GWTFolder) {
                        ...
                        if (!((GWTDocument) obj).getActualVersion().getName().equals(mapFilter.get(key).getFilterValue1())) {
                            add = false;
                        }
                    } else if (obj instanceof GWTMail) {
                        add = false;
