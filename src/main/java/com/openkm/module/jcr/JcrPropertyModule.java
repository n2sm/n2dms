...
public class JcrPropertyModule implements PropertyModule {
    private static Logger log = LoggerFactory
            .getLogger(JcrPropertyModule.class);

    @Override
    public void addCategory(final String token, final String nodePath,
            final String catId) throws VersionException, LockException,
        ...
        try {
            ...
            // Check subscriptions
            BaseNotificationModule.checkSubscriptions(node,
                    session.getUserID(), "ADD_CATEGORY", null);

            // Check scripting
            BaseScriptingModule.checkScripts(session, node, node,
                    "ADD_CATEGORY");

            // Activity log
            UserActivity.log(session.getUserID(), "ADD_CATEGORY",
                    node.getUUID(), nodePath, catId);
    ...
    @Override
    public void removeCategory(final String token, final String nodePath,
            final String catId) throws VersionException, LockException,
            PathNotFoundException, AccessDeniedException, RepositoryException,
            DatabaseException {
        ...
        try {
            ...
            // Check subscriptions
            BaseNotificationModule.checkSubscriptions(node,
                    session.getUserID(), "REMOVE_CATEGORY", null);
            ...
            // Activity log
            UserActivity.log(session.getUserID(), "REMOVE_CATEGORY",
                    node.getUUID(), nodePath, catId);
        } catch (final javax.jcr.PathNotFoundException e) {
            JCRUtils.discardsPendingChanges(node);
            throw new PathNotFoundException(e.getMessage(), e);
    ...
    @Override
    public String addKeyword(final String token, final String nodePath,
            String keyword) throws VersionException, LockException,
            PathNotFoundException, AccessDeniedException, RepositoryException,
            DatabaseException {
        ...
        try {
            ...
            if (Config.USER_KEYWORDS_CACHE) {
                UserNodeKeywordsManager.add(session.getUserID(),
                        node.getUUID(), keyword);
            }

            // Check subscriptions
            BaseNotificationModule.checkSubscriptions(node,
                    session.getUserID(), "ADD_KEYWORD", null);

            // Check scripting
            BaseScriptingModule
                    .checkScripts(session, node, node, "ADD_KEYWORD");

            // Activity log
            UserActivity.log(session.getUserID(), "ADD_KEYWORD",
                    node.getUUID(), nodePath, keyword);
        } catch (final javax.jcr.PathNotFoundException e) {
            JCRUtils.discardsPendingChanges(node);
            throw new PathNotFoundException(e.getMessage(), e);
        } catch (final javax.jcr.AccessDeniedException e) {
    ...
    @Override
    public void removeKeyword(final String token, final String nodePath,
            final String keyword) throws VersionException, LockException,
            PathNotFoundException, AccessDeniedException, RepositoryException,
            DatabaseException {
        ...
        try {
            ...
            // Update cache
            if (Config.USER_KEYWORDS_CACHE) {
                UserNodeKeywordsManager.remove(session.getUserID(),
                        node.getUUID(), keyword);
            }

            // Check subscriptions
            BaseNotificationModule.checkSubscriptions(node,
                    session.getUserID(), "REMOVE_KEYWORD", null);
            ...
            // Activity log
            UserActivity.log(session.getUserID(), "REMOVE_KEYWORD",
                    node.getUUID(), nodePath, keyword);
        } catch (final javax.jcr.PathNotFoundException e) {
            JCRUtils.discardsPendingChanges(node);
            throw new PathNotFoundException(e.getMessage(), e);
