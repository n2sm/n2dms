...
public class BaseFolderModule {
    ...
    public static Node create(final Session session, final Node parentNode,
            final String name) throws javax.jcr.ItemExistsException,
        ...
        final Node folderNode = parentNode.addNode(name, Folder.TYPE);
        folderNode.setProperty(Property.KEYWORDS, new String[] {});
        folderNode.setProperty(Property.CATEGORIES, new String[] {},
     ...
     */
    public static Folder getProperties(final Session session, final Node fldNode)
        ...
        for (final NodeIterator nit = fldNode.getNodes(); nit.hasNext();) {
        ...
        if (Config.SYSTEM_READONLY) {
            ...
            final Path path = ((NodeImpl) fldNode).getPrimaryPath();
        ...
        if (fldNode.isNodeType(Notification.TYPE)) {
            final Value[] subscriptors = fldNode.getProperty(
        ...
        final Value[] keywords = fldNode.getProperty(Property.KEYWORDS)
                .getValues();
        ...
        final Value[] categories = fldNode.getProperty(Property.CATEGORIES)
                .getValues();
        ...
        if (fldNode.isNodeType(Note.MIX_TYPE)) {
            ...
            final Node notesNode = fldNode.getNode(Note.LIST);
     ...
     */
    public static void copy(final Session session, final Node srcFolderNode,
            final Node dstFolderNode) throws NoSuchNodeTypeException,
        ...
        log.debug(
                "copy({}, {}, {})",
                new Object[] { session, srcFolderNode.getPath(),
                        dstFolderNode.getPath() });

        for (final NodeIterator it = srcFolderNode.getNodes(); it.hasNext();) {
     ...
     */
    public static ContentInfo getContentInfo(final Node folderNode)
        ...
        try {
            for (final NodeIterator ni = folderNode.getNodes(); ni.hasNext();) {
                ...
                if (child.isNodeType(Folder.TYPE)) {
                    final ContentInfo ci = getContentInfo(child);
                    contentInfo.setFolders(contentInfo.getFolders()
                            + ci.getFolders() + 1);
                    ...
                    final Node documentContentNode = child
                            .getNode(Document.CONTENT);
                    final long size = documentContentNode.getProperty(
                            Document.SIZE).getLong();
                    contentInfo.setDocuments(contentInfo.getDocuments() + 1);
                    contentInfo.setSize(contentInfo.getSize() + size);
                } else if (child.isNodeType(Mail.TYPE)) {
                    final long size = child.getProperty(Mail.SIZE).getLong();
     ...
     */
    public static boolean hasLockedNodes(final Node node)
     ...
     */
    public static boolean hasWriteAccess(final Node node)
            throws javax.jcr.RepositoryException {
        ...
        final AccessManager am = ((SessionImpl) node.getSession())
                .getAccessManager();

        for (final NodeIterator ni = node.getNodes(); ni.hasNext();) {
            final Node child = ni.nextNode();
            final Path path = ((NodeImpl) node).getPrimaryPath();
     ...
     */
    public static void purge(final Session session, final Node fldNode)
            throws VersionException, javax.jcr.lock.LockException,
            ConstraintViolationException, javax.jcr.RepositoryException {
        ...
        final String author = fldNode.getProperty(Folder.AUTHOR).getString();
        fldNode.remove();
     ...
     */
    public static void getContentInfoVisitor(final Session session,
            final Node node) throws NoSuchNodeTypeException, VersionException,
            ConstraintViolationException, LockException, RepositoryException {
        log.debug("getContentInfoVisitor({}, {})",
                new Object[] { session, node });
        final ContentInfo ci = new ContentInfo();

        final TraversingItemVisitor visitor = new TraversingItemVisitor.Default() {
            @Override
            protected void entering(final Node node, final int level)
                ...
                if (node.isNodeType(Folder.TYPE)) {
                    ...
                    final Node docContent = node.getNode(Document.CONTENT);
                    final long size = docContent.getProperty(Document.SIZE)
                            .getLong();
                    ci.setDocuments(ci.getDocuments() + 1);
                    ci.setSize(ci.getSize() + size);
                } else if (node.isNodeType(Mail.TYPE)) {
                    final long size = node.getProperty(Mail.SIZE).getLong();
