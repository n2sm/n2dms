...
public class DbPropertyModule implements PropertyModule {
    ...
    public void addCategory(final String token, final String nodePath,
        ...
        try {
            ...
            final String uuid = NodeBaseDAO.getInstance().getUuidFromPath(
                    nodePath);
            final NodeBase nNode = NodeBaseDAO.getInstance().findByPk(uuid);
            NodeBaseDAO.getInstance().addCategory(uuid, catId);

            // Check subscriptions
            BaseNotificationModule.checkSubscriptions(nNode, auth.getName(),
                    "ADD_CATEGORY", null);
            ...
            // Activity log
            UserActivity.log(auth.getName(), "ADD_CATEGORY", uuid, nodePath,
                    catId);
    ...
    public void removeCategory(final String token, final String nodePath,
        ...
        try {
            ...
            // Activity log
            UserActivity.log(auth.getName(), "REMOVE_CATEGORY", uuid, nodePath,
                    catId);
        } catch (final DatabaseException e) {
            throw e;
        } finally {
    ...
    public String addKeyword(final String token, final String nodePath,
        ...
        try {
            ...
            if (keyword != null) {
                ...
                // Check subscriptions
                BaseNotificationModule.checkSubscriptions(nNode,
                        auth.getName(), "ADD_KEYWORD", null);

                // Check scripting
                //BaseScriptingModule.checkScripts(session, documentNode, documentNode, "ADD_KEYWORD");

                // Activity log
                UserActivity.log(auth.getName(), "ADD_KEYWORD", uuid, nodePath,
                        keyword);
            }
        } catch (final DatabaseException e) {
            throw e;
    ...
    public void removeKeyword(final String token, final String nodePath,
        ...
        try {
            ...
            // Activity log
            UserActivity.log(auth.getName(), "REMOVE_KEYWORD", uuid, nodePath,
                    keyword);
        } catch (final DatabaseException e) {
            throw e;
        } finally {
