...
public class WorkspaceUserProperties {
    ...
    final AsyncCallback<GWTWorkspace> callbackGetUserWorkspace = new AsyncCallback<GWTWorkspace>() {
        @Override
        public void onSuccess(final GWTWorkspace result) {
            workspace = result;
            user = result.getUser();
            ...
            Main.get().mainPanel.bottomPanel.userInfo.setUser(
                    user.getUsername(), result.isTabAdminVisible());
            ...
            Main.get().mainPanel.dashboard.startRefreshingDashboard(workspace
                    .getDashboardSchedule());
            Main.get().mainPanel.topPanel.mainMenu
                    .startRefreshingMenus(workspace.getDashboardSchedule());
            ...
            // Show / hide menus
            Main.get().mainPanel.topPanel.mainMenu.setFileMenuVisible(workspace
                    .isMenuFileVisible());
            Main.get().mainPanel.topPanel.mainMenu.setEditMenuVisible(workspace
                    .isMenuEditVisible());
            Main.get().mainPanel.topPanel.mainMenu
                    .setToolsMenuVisible(workspace.isMenuToolsVisible());
            Main.get().mainPanel.topPanel.mainMenu
                    .setBookmarkMenuVisible(workspace.isMenuBookmarksVisible());
            Main.get().mainPanel.topPanel.mainMenu
                    .setTemplatesMenuVisible(workspace.isMenuTemplatesVisible());
            Main.get().mainPanel.topPanel.mainMenu.setHelpMenuVisible(workspace
                    .isMenuHelpVisible());

            // Init available languages
            Main.get().mainPanel.topPanel.mainMenu
                    .initAvailableLanguage(workspace.getLangs());
            // Init available templates
            Main.get().mainPanel.topPanel.mainMenu.refreshAvailableTemplates();

            // Enabling / disabling some actions
            Main.get().mainPanel.topPanel.toolBar.setAvailableOption(workspace
                    .getProfileToolbar());
            ...
            if (result.isTabAdminVisible()) {
                Main.get().mainPanel.topPanel.mainMenu.administration
                        .setVisible(true);
                Main.get().mainPanel.topPanel.tabWorkspace.showAdministration();
                refreshTab = true;
            }
            refreshTab = Main.get().mainPanel.topPanel.tabWorkspace
                    .showExtensionTabs() || refreshTab;
            if (refreshTab) {
            ...
            if (workspace.isStackTemplatesVisible()) {
                ...
                Main.get().mainPanel.desktop.navigator.taxonomyTree.folderSelectPopup
                        .showTemplates();
                Main.get().mainPanel.desktop.navigator.categoriesTree.folderSelectPopup
                        .showTemplates();
                Main.get().mainPanel.desktop.navigator.thesaurusTree.folderSelectPopup
                        .showTemplates();
                Main.get().mainPanel.desktop.navigator.personalTree.folderSelectPopup
                        .showTemplates();
                Main.get().mainPanel.desktop.navigator.templateTree.folderSelectPopup
                        .showTemplates();
                Main.get().mainPanel.desktop.navigator.mailTree.folderSelectPopup
                        .showTemplates();
                Main.get().mainPanel.desktop.navigator.trashTree.folderSelectPopup
                        .showTemplates();
            ...
            Main.get().mainPanel.search.searchBrowser.searchIn.searchControl
                    .setMinSearchCharacters(workspace.getMinSearchCharacters());

            // Filebrowser profile
            Main.get().mainPanel.desktop.browser.fileBrowser
                    .setProfileFileBrowser(workspace.getProfileFileBrowser(),
            ...
            getUINotificationMessages(new Long(
                    Main.get().workspaceUserProperties.workspace
                            .getUINotificationSchedule() / 2).intValue());
        }
    ...
    final AsyncCallback<Double> callbackGetUserDocumentsSize = new AsyncCallback<Double>() {
        ...
        public void onSuccess(final Double result) {
            Main.get().mainPanel.bottomPanel.userInfo
                    .setUserRepositorySize(result.longValue());
        }
    ...
    private void getUINotificationMessages(final int scheduleTime) {
        uINotificationService.get(new AsyncCallback<List<GWTUINotification>>() {
            @Override
            public void onSuccess(final List<GWTUINotification> result) {
                Main.get().mainPanel.bottomPanel.userInfo.reset();
                boolean schedule = true;

                if (!result.isEmpty()) {
                    for (final GWTUINotification uin : result) {
                        Main.get().mainPanel.bottomPanel.userInfo
                                .addUINotification(uin);
                    ...
                    Main.get().mainPanel.bottomPanel.userInfo
                            .setLastUIId(result.get(result.size() - 1).getId());
                }
    ...
    public void setAvailableAction() {
        if (Main.get().workspaceUserProperties.getWorkspace().isStackTaxonomy()) {
            Main.get().mainPanel.desktop.navigator.taxonomyTree.menuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
            Main.get().mainPanel.desktop.browser.fileBrowser.taxonomyMenuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
        }
        if (Main.get().workspaceUserProperties.getWorkspace()
                .isStackCategoriesVisible()) {
            Main.get().mainPanel.desktop.navigator.categoriesTree.menuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
            Main.get().mainPanel.desktop.browser.fileBrowser.categoriesMenuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
        }
        if (Main.get().workspaceUserProperties.getWorkspace()
                .isStackThesaurusVisible()) {
            Main.get().mainPanel.desktop.navigator.thesaurusTree.menuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
            Main.get().mainPanel.desktop.browser.fileBrowser.thesaurusMenuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
        }
        if (Main.get().workspaceUserProperties.getWorkspace()
                .isStackTemplatesVisible()) {
            Main.get().mainPanel.desktop.navigator.templateTree.menuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
            Main.get().mainPanel.desktop.browser.fileBrowser.templatesMenuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
        }
        if (Main.get().workspaceUserProperties.getWorkspace()
                .isStackPersonalVisible()) {
            Main.get().mainPanel.desktop.navigator.personalTree.menuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
            Main.get().mainPanel.desktop.browser.fileBrowser.personalMenuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
        }
        if (Main.get().workspaceUserProperties.getWorkspace()
                .isStackMailVisible()) {
            Main.get().mainPanel.desktop.navigator.mailTree.menuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
            Main.get().mainPanel.desktop.browser.fileBrowser.mailMenuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
        }
        if (Main.get().workspaceUserProperties.getWorkspace()
                .isStackTrashVisible()) {
            Main.get().mainPanel.desktop.navigator.trashTree.menuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
            Main.get().mainPanel.desktop.browser.fileBrowser.trashMenuPopup.menu
                    .setAvailableOption(workspace.getAvailableOption());
        }
