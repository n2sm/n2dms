...
public class Status extends PopupPanel {
     ...
     */
    public void refresh() {
        if (flag_merge || flag_setCategories || flag_addKeywords
                || flag_addNotes || flag_removeCategories
                || flag_removeKeywords || flag_addPropertyGroup
                || flag_removePropertyGroup || flag_delete || flag_copy
                || flag_move || flag_omr) {
            final int left = widget.getAbsoluteLeft()
                    + (widget.getOffsetWidth() - 200) / 2;
            final int top = widget.getAbsoluteTop()
                    + (widget.getOffsetHeight() - 40) / 2;
