    ...
    public FolderTextBox() {
        ...
        textBox.addKeyUpHandler(new KeyUpHandler() {
            ...
            public void onKeyUp(final KeyUpEvent event) {
                final int action = Main.get().activeFolderTree
                        .getFolderAction();
                ...
                case (char) KeyCodes.KEY_ENTER:
                    ...
                    case FolderTree.ACTION_CREATE:
                        Main.get().activeFolderTree.rootItem
                                .addStyleName("okm-DisableSelect"); // Disables drag and drop browser text selection
                        if (textBox.getText().length() > 0) {
                            Main.get().activeFolderTree.create(textBox
                                    .getText());
                        } else {
                    ...
                    case FolderTree.ACTION_RENAME:
                        Main.get().activeFolderTree.rootItem
                                .addStyleName("okm-DisableSelect"); // Disables drag and drop browser text selection
                        if (textBox.getText().length() > 0) {
                            Main.get().activeFolderTree.rename(textBox
                                    .getText());
                        } else {
        ...
        panel.add(new HTML(Util.imageItemHTML("img/menuitem_empty.gif", "",
                "top")));
        panel.add(textBox);
        initWidget(panel);
    }
