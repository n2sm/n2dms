...
public class PropertyGroupPopup extends DialogBox {
    ...
    public PropertyGroupPopup() {
        ...
        final HorizontalPanel grpNamePanel = new HorizontalPanel();
        propertyGroupName = new HTML("");
        grpNamePanel.add(propertyGroupName);
        grpNamePanel.setWidth("100%");
        grpNamePanel.setCellHorizontalAlignment(propertyGroupName,
        ...
        hPanel.setCellHorizontalAlignment(cancel,
                HasHorizontalAlignment.ALIGN_CENTER);
        hPanel.setCellHorizontalAlignment(add,
                HasHorizontalAlignment.ALIGN_CENTER);
        ...
        table.getCellFormatter().addStyleName(1, 0,
        ...
        table.getCellFormatter().setHorizontalAlignment(2, 0,
                HasHorizontalAlignment.ALIGN_CENTER);
        table.getCellFormatter().setHorizontalAlignment(3, 0,
                HasHorizontalAlignment.ALIGN_CENTER);
     ...
     * drawPhase
     */
    private void switchPhase(final int phase) {
        this.phase = phase;
        switch (phase) {
    ...
    private void getAllGroups() {
        if (!Main.get().mainPanel.desktop.browser.fileBrowser.isMassive()) {
            ...
            if (!path.equals("")) {
                propertyGroupService.getAllGroups(path,
                        new AsyncCallback<List<GWTPropertyGroup>>() {
                            ...
                            public void onSuccess(
                                ...
                                for (final GWTPropertyGroup group : result) {
                                    listBox.addItem(group.getLabel(),
                                            group.getName());
                                }
                            }
            ...
            propertyGroupService
                    .getAllGroups(new AsyncCallback<List<GWTPropertyGroup>>() {
                        @Override
                        public void onSuccess(
                                final List<GWTPropertyGroup> result) {
                            listBox.clear();
        ...
        if (listBox.getSelectedIndex() > 0) {
            ...
            if (phase == PHASE_SHOW_PROPERTIES) {
                if (manager.getValidationProcessor().validate()) {
                    ...
                    propertyGroupService.addGroup(path, grpName,
                            new AsyncCallback<Object>() {
                                @Override
                                public void onSuccess(final Object result) {
                                    // Adding properties
                                    propertyGroupService.setProperties(
                                            path,
                                            grpName,
                ...
                propertyGroupName.setHTML(listBox.getItemText(listBox
                        .getSelectedIndex()));
                propertyGroupService.getPropertyGroupForm(grpName,
                        new AsyncCallback<List<GWTFormElement>>() {
                            @Override
                            public void onSuccess(
                                    final List<GWTFormElement> result) {
                            ...
                            @Override
                            public void onFailure(final Throwable caught) {
                                Main.get().showError("getPropertyGroupForm",
                                        caught);
                            }
                        });
     ...
     */
    private void refreshingActualNode(final List<GWTFormElement> formElements) {
        if (Main.get().mainPanel.desktop.browser.fileBrowser.isMassive()) {
            ...
            final Object node = Main.get().mainPanel.topPanel.toolBar
                    .getActualNode();
            if (node != null) {
                if (Main.get().mainPanel.topPanel.toolBar.isNodeDocument()) {
                    final GWTDocument doc = (GWTDocument) Main.get().mainPanel.topPanel.toolBar
                            .getActualNode();
                    if (Main.get().mainPanel.desktop.browser.fileBrowser
                            .isPanelSelected()
                    ...
                    final GWTFolder folder = (GWTFolder) Main.get().mainPanel.topPanel.toolBar
                            .getActualNode();
                    if (Main.get().mainPanel.desktop.browser.fileBrowser
                            .isPanelSelected()
                            && Main.get().mainPanel.desktop.browser.fileBrowser
                    ...
                    final GWTMail mail = (GWTMail) Main.get().mainPanel.topPanel.toolBar
                            .getActualNode();
                    if (Main.get().mainPanel.desktop.browser.fileBrowser
                            .isPanelSelected()
                            && Main.get().mainPanel.desktop.browser.fileBrowser
