...
public class DatabaseRecordSelectPopup extends DialogBox {
     ...
     * DatabaseRecordSelectPopup
     */
    public DatabaseRecordSelectPopup(final GWTSuggestBox suggestBox,
            final HasDatabaseRecord databaseRecord, final HasSearch search) {
        ...
        recordTabla.addClickHandler(new ClickHandler() {
            ...
            public void onClick(final ClickEvent event) {
                markSelectedRow(recordTabla.getCellForEvent(event)
                        .getRowIndex());
        ...
        acceptButton = new Button(Main.i18n("button.accept"),
                new ClickHandler() {
                    ...
                    public void onClick(final ClickEvent event) {
                        if (selectedRow >= 0) {
                            databaseRecord.setKeyValue(rowKeyValueMap
                                    .get(selectedRow));
                            // Updating suggestbox values ( for when value will be update )
                            suggestBox.setValue(rowKeyValueMap.get(selectedRow)
                                    .getKey());
                            suggestBox.setText(rowKeyValueMap.get(selectedRow)
                                    .getValue());
                            if (search != null) {
        ...
        if (suggestBox.getFilterMinLen() > 0) {
            final HorizontalPanel hInfoPanel = new HorizontalPanel();
            final HTML filterInfo = new HTML(MessageFormat.format(
                    Main.i18n("form.manager.suggestbox.min.filter"),
                    suggestBox.getFilterMinLen()));
            final HTML space = Util.hSpace("5");
            hInfoPanel.add(filterInfo);
            hInfoPanel.add(space);
            vPanel.add(hInfoPanel);
            vPanel.setCellHorizontalAlignment(hInfoPanel,
        ...
        vPanel.setCellVerticalAlignment(record,
                HasVerticalAlignment.ALIGN_MIDDLE);
        vPanel.setCellVerticalAlignment(hPanel,
                HasVerticalAlignment.ALIGN_MIDDLE);
        vPanel.setCellHorizontalAlignment(record,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(scrollDatabaseRecordPanel,
                HasHorizontalAlignment.ALIGN_CENTER);
    ...
    private void findFilteredDatabaseRecords() {
        ...
        keyValueService.getKeyValues(
                tables,
                MessageFormat.format(suggestBox.getFilterQuery(),
                        record.getText()),
                new AsyncCallback<List<GWTKeyValue>>() {
                    ...
                    public void onSuccess(final List<GWTKeyValue> result) {
                        ...
                        for (final GWTKeyValue keyValue : result) {
                            final int row = recordTabla.getRowCount();
    ...
    private void styleRow(final int row, final boolean selected) {
        if (row >= 0) {
            if (selected) {
                recordTabla.getRowFormatter().addStyleName(row,
                        "okm-Table-SelectedRow");
            } else {
                recordTabla.getRowFormatter().removeStyleName(row,
                        "okm-Table-SelectedRow");
            }
        }
    }
