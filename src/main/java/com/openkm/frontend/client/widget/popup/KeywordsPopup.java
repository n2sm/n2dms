...
public class KeywordsPopup extends DialogBox {
    ...
    public KeywordsPopup() {
            ...
            public void onKeyUp(final KeyUpEvent event) {
                if ((char) KeyCodes.KEY_ENTER == event.getNativeKeyCode()
                    ...
                    Main.get().mainPanel.enableKeyShorcuts(); // Enables general
                                                              // keys
                                                              // applications
                    final String keys[] = suggestKey.getText().split(" "); // Separates
                                                                           // keywords
                                                                           // by
                                                                           // space
        ...
        table.setHTML(2, 0, "<b>" + Main.i18n("document.keywords.cloud")
                + "</b>");
    ...
    public void reset() {
        ...
        if (Main.get().workspaceUserProperties.getWorkspace()
                .getAvailableOption().isRemoveKeywordOption()) {
            remove = true;
        } else {
            remove = false;
        }
        ...
        if (!Main.get().mainPanel.desktop.browser.fileBrowser.isMassive()) {
            ...
            if (Main.get().mainPanel.desktop.browser.fileBrowser
                ...
                if (Main.get().mainPanel.desktop.browser.fileBrowser
                        .isDocumentSelected()) {
                    docKeywords = Main.get().mainPanel.desktop.browser.tabMultiple.tabDocument.document
                            .getKeywords();
                } else if (Main.get().mainPanel.desktop.browser.fileBrowser
                        .isFolderSelected()) {
                    docKeywords = Main.get().mainPanel.desktop.browser.tabMultiple.tabFolder.folder
                            .getKeywords();
                } else if (Main.get().mainPanel.desktop.browser.fileBrowser
                        .isMailSelected()) {
                    docKeywords = Main.get().mainPanel.desktop.browser.tabMultiple.tabMail.mail
                            .getKeywords();
                }
            } else {
    ...
    public void addPendingKeyWordsList() {
        if (!keyWordsListPending.isEmpty()) {
            ...
            if (!keywordMap.containsKey(keyword) && keyword.length() > 0) {
                ...
                final Widget keywordButton = getKeyWidget(keyword, remove); // Always
                // allow
                // remove
                // added
                // keyword
