...
public class UserPopup extends DialogBox implements ClickHandler {
    ...
    public UserPopup() {
        ...
        imapPassword = new HTML(
                Main.i18n("user.preferences.imap.user.password"));
        ...
        imapPassordError = new HTML(
                Main.i18n("user.preferences.imap.password.error.void"));
        ...
        update = new Button(Main.i18n("button.update"), new ClickHandler() {
            ...
            public void onClick(final ClickEvent event) {
                ...
                if (!userPasswordText.getText().equals(
                        userPasswordTextVerify.getText())) {
                    passwordError.setVisible(true);
                    // Case creation
                } else if (Main.get().workspaceUserProperties.getWorkspace()
                        .getImapID() < 0
                        && imapUserPasswordText.getText().equals("")
                        && (imapFolderText.getText().length() > 0
                                || imapUserText.getText().length() > 0 || hostText
                                .getText().length() > 0)) {
                    imapPassordError.setVisible(true);
                    // Case update
                } else if ((imapUserPasswordText.getText().length() > 0
                        || imapFolderText.getText().length() > 0
                        || imapUserText.getText().length() > 0 || hostText
                        .getText().length() > 0)
                        && !(imapFolderText.getText().length() > 0
                    ...
                    workspace.setUser(Main.get().workspaceUserProperties
                            .getUser());
                    ...
                    workspace.setImapID(Main.get().workspaceUserProperties
                            .getWorkspace().getImapID());
                    // First must validate password
                    workspaceService.isValidPassword(
                            userPasswordText.getText(),
                            new AsyncCallback<String>() {
                                @Override
                                public void onSuccess(final String result) {
                                    if (result.equals("")) {
                                        workspaceService.updateUserWorkspace(
                                                workspace,
                                                callbackUpdateUserWorkspace);
                                    } else {
                                ...
                                public void onFailure(final Throwable caught) {
                                    Main.get().showError(
                                            "callbackIsValidPassword", caught);
                                }
                            });
                }
            }
        ...
        test = new Button(Main.i18n("button.test"), new ClickHandler() {
            ...
            public void onClick(final ClickEvent event) {
                ...
                generalService.testImapConnection(hostText.getText(),
                        imapUserText.getText(), imapUserPasswordText.getText(),
                        imapFolderText.getText(),
                        new AsyncCallback<GWTTestImap>() {
                            @Override
                            public void onSuccess(final GWTTestImap result) {
                                if (!result.isError()) {
                                    imapTestError.setVisible(false);
                                    imapTestOK.setVisible(true);
                                } else {
                                    imapTestError.setHTML(Main
                                            .i18n("user.preferences.imap.test.error")
                                            + "<br>" + result.getErrorMsg());
                            ...
                            public void onFailure(final Throwable caught) {
                                ...
                                imapTestOK.setVisible(false);
                                test.setEnabled(true);
                                Main.get().showError("testImapConnection",
                                        caught);
                            }
                        });
            }
        ...
        delete = new Button(Main.i18n("button.delete"), new ClickHandler() {
            @Override
            public void onClick(final ClickEvent event) {
                final long Id = Main.get().workspaceUserProperties
                        .getWorkspace().getImapID();

                if (Id >= 0) {
                    workspaceService.deleteMailAccount(Id,
                            callbackDeleteMailAccount);
                }
            }
        });
        ...
        userFlexTable.getFlexCellFormatter().setVerticalAlignment(4, 0,
        ...
        mailFlexTable.getFlexCellFormatter().setAlignment(5, 1,
                HasHorizontalAlignment.ALIGN_CENTER,
                HasVerticalAlignment.ALIGN_MIDDLE);
        mailFlexTable.getFlexCellFormatter().setAlignment(5, 2,
                HasHorizontalAlignment.ALIGN_CENTER,
                HasVerticalAlignment.ALIGN_MIDDLE);
        ...
        vPanel.setCellHorizontalAlignment(userGroupBoxPanel,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(mailGroupBoxPanel,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(hPanel,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(passwordError,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(passwordValidationError,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(imapPassordError,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(imapError,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(imapTestError,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(imapTestOK,
                HasHorizontalAlignment.ALIGN_CENTER);
    ...
    public void langRefresh() {
        ...
        imapPassordError.setHTML(Main
                .i18n("user.preferences.imap.password.error.void"));
    ...
    public void show() {
        ...
        for (final String string : workspace.getRoleList()) {
            rolesPanel.add(new HTML(string));
        }
        ...
        if (!workspace.isStackMailVisible()) {
            ...
            vPanel.remove(imapPassordError);
            vPanel.remove(imapError);
            vPanel.remove(imapTestError);
            vPanel.remove(imapTestOK);
