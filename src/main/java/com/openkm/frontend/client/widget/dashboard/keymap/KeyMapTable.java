...
public class KeyMapTable extends Composite {
    ...
    private void addDocumentRow(final GWTQueryResult gwtQueryResult,
            final Score score) {
        final Collection<String> selectedKeyList = Main.get().mainPanel.dashboard.keyMapDashboard
                .getFiltering();
        int rows = table.getRowCount();
        final int firstRow = rows;
        ...
        final Image gotoDocument = new Image(
                "img/icon/actions/goto_document.gif");
        ...
        if (doc.isAttachment()) {
            final SimplePanel sp = new SimplePanel(); // Solves some middle alignament problem derived from mimeImageHTML method
        ...
        // Format
        table.getCellFormatter().setHorizontalAlignment(rows, 0,
                HasHorizontalAlignment.ALIGN_CENTER);
        table.getCellFormatter().setHorizontalAlignment(rows, 1,
                HasHorizontalAlignment.ALIGN_RIGHT);
        table.getCellFormatter().setHorizontalAlignment(rows, 2,
                HasHorizontalAlignment.ALIGN_LEFT);
        table.getCellFormatter().setVerticalAlignment(rows, 0,
                HasVerticalAlignment.ALIGN_MIDDLE);
        table.getCellFormatter().setVerticalAlignment(rows, 0,
                HasVerticalAlignment.ALIGN_MIDDLE);
        table.getCellFormatter().setVerticalAlignment(rows, 2,
                HasVerticalAlignment.ALIGN_MIDDLE);
        ...
        // Writing detail
        rows++; // Next row line
        final FlexTable tableDocument = new FlexTable();
        final FlexTable tableProperties = new FlexTable();
        final FlexTable tableSubscribedUsers = new FlexTable();
        tableDocument.setWidget(0, 0, tableProperties);
        tableDocument.setHTML(0, 1, "");
        tableDocument.setWidget(0, 2, tableSubscribedUsers);
        tableDocument.getFlexCellFormatter().setVerticalAlignment(0, 0,
                HasVerticalAlignment.ALIGN_TOP);
        tableDocument.getFlexCellFormatter().setVerticalAlignment(0, 2,
                HasVerticalAlignment.ALIGN_TOP);
        ...
        tableProperties.setHTML(0, 0, "<b>" + Main.i18n("document.folder")
                + "</b>");
        tableProperties.setHTML(0, 1, doc.getParentPath());
        tableProperties.setHTML(1, 0, "<b>" + Main.i18n("document.size")
                + "</b>");
        tableProperties.setHTML(1, 1,
                Util.formatSize(doc.getActualVersion().getSize()));
        tableProperties.setHTML(2, 0, "<b>" + Main.i18n("document.created")
                + "</b>");
        ...
        tableProperties.setHTML(2, 1,
                dtf.format(doc.getCreated()) + " " + Main.i18n("document.by")
                        + " " + doc.getAuthor());
        tableProperties.setHTML(3, 0,
                "<b>" + Main.i18n("document.lastmodified") + "</b>");
        tableProperties.setHTML(3, 1, dtf.format(doc.getLastModified()) + " "
                + Main.i18n("document.by") + " "
                + doc.getActualVersion().getAuthor());
        tableProperties.setHTML(4, 0, "<b>" + Main.i18n("document.mimetype")
                + "</b>");
        tableProperties.setHTML(4, 1, doc.getMimeType());
        tableProperties.setHTML(5, 0, "<b>" + Main.i18n("document.status")
                + "</b>");
        if (doc.isCheckedOut()) {
            tableProperties.setHTML(5, 1, Main.i18n("document.status.checkout")
                    + " " + doc.getLockInfo().getOwner());
        } else if (doc.isLocked()) {
            tableProperties.setHTML(5, 1, Main.i18n("document.status.locked")
                    + " " + doc.getLockInfo().getOwner());
        } else {
            tableProperties.setHTML(5, 1, Main.i18n("document.status.normal"));
        }
        tableProperties.setHTML(6, 0, "<b>" + Main.i18n("document.subscribed")
                + "</b>");
        if (doc.isSubscribed()) {
        ...
        // Setting subscribers
        tableSubscribedUsers.setHTML(0, 0,
                "<b>" + Main.i18n("document.subscribed.users") + "<b>");
        ...
        for (final GWTUser subscriptor : doc.getSubscriptors()) {
            ...
            setRowWordWarp(tableSubscribedUsers.getRowCount() - 1, 1, false,
                    tableSubscribedUsers);
        }

        final HorizontalPanel hKeyPanel = addKeywords(table, doc.getKeywords(),
                selectedKeyList); // Drawing keywords
        ...
        case VISIBLE_SMALL:
            ...
            table.getCellFormatter().addStyleName(firstRow, 0,
                    "okm-Table-BottomBorder");
            table.getCellFormatter().addStyleName(firstRow, 1,
                    "okm-Table-BottomBorder");
            table.getCellFormatter().addStyleName(firstRow, 2,
                    "okm-Table-BottomBorder");
            break;
        case VISIBLE_MEDIUM:
            tableDocument.setVisible(false);
    ...
    public void langRefresh() {
        // Documents
        for (final FlexTable tableDocument : tableDocumentList) {
            final FlexTable tableProperties = (FlexTable) tableDocument
                    .getWidget(0, 0);
            final FlexTable tableSubscribedUsers = (FlexTable) tableDocument
                    .getWidget(0, 2);

            tableProperties.setHTML(1, 0, "<b>" + Main.i18n("document.folder")
                    + "</b>");
            tableProperties.setHTML(2, 0, "<b>" + Main.i18n("document.size")
                    + "</b>");
            tableProperties.setHTML(3, 0, "<b>" + Main.i18n("document.created")
                    + "</b>");
            tableProperties.setHTML(4, 0,
                    "<b>" + Main.i18n("document.lastmodified") + "</b>");
            tableProperties.setHTML(5, 0,
                    "<b>" + Main.i18n("document.mimetype") + "</b>");
            tableProperties.setHTML(6, 0, "<b>" + Main.i18n("document.status")
                    + "</b>");
            tableProperties.setHTML(7, 0,
                    "<b>" + Main.i18n("document.subscribed") + "</b>");

            tableSubscribedUsers.setHTML(0, 0,
                    "<b>" + Main.i18n("document.subscribed.users") + "<b>");
        }
        // Mails
        for (final FlexTable tableMail : tableMailList) {
            final FlexTable tableProperties = (FlexTable) tableMail.getWidget(
                    0, 0);

            tableProperties.setHTML(0, 0, "<b>" + Main.i18n("mail.folder")
                    + "</b>");
            tableProperties.setHTML(1, 0, "<b>" + Main.i18n("mail.size")
                    + "</b>");
            tableProperties.setHTML(2, 0, "<b>" + Main.i18n("mail.created")
                    + "</b>");
            tableProperties.setHTML(3, 0, "<b>" + Main.i18n("mail.mimetype")
                    + "</b>");
        }
    }
     ...
     */
    public void changeVisibilityDetail(final int value) {
        visibleStatus = value;
    ...
    private void refreshingVisibilityDetail() {
        ...
        if (visibleStatus == VISIBLE_SMALL) {
            for (final String row : firtRowList) {
                final int firstRow = Integer.parseInt(row);
                table.getCellFormatter().addStyleName(firstRow, 0,
                        "okm-Table-BottomBorder");
                table.getCellFormatter().addStyleName(firstRow, 1,
                        "okm-Table-BottomBorder");
            ...
            for (final String row : firtRowList) {
                final int firstRow = Integer.parseInt(row);
                table.getCellFormatter().removeStyleName(firstRow, 0,
                        "okm-Table-BottomBorder");
                table.getCellFormatter().removeStyleName(firstRow, 1,
                        "okm-Table-BottomBorder");
                table.getCellFormatter().removeStyleName(firstRow, 2,
                        "okm-Table-BottomBorder");
            }
        }
        ...
        for (final FlexTable tableFolder : tableFolderList) {
            // Setting visibility
        ...
        for (final HorizontalPanel hKeyPanel : hKeyPanelList) {
            switch (visibleStatus) {
            case VISIBLE_SMALL:
                hKeyPanel.setVisible(false);
                break;
    ...
    private void addFolderRow(final GWTQueryResult gwtQueryResult,
        ...
        final Image gotoFolder = new Image("img/icon/actions/goto_folder.gif");
        ...
        // Writing detail
        rows++; // Next row line
        final FlexTable tableFolder = new FlexTable();
        final FlexTable tableProperties = new FlexTable();
        final FlexTable tableSubscribedUsers = new FlexTable();
        tableFolder.setWidget(0, 0, tableProperties);
        tableFolder.setHTML(0, 1, "");
        tableFolder.setWidget(0, 2, tableSubscribedUsers);
        tableFolder.getFlexCellFormatter().setVerticalAlignment(0, 0,
                HasVerticalAlignment.ALIGN_TOP);
        tableFolder.getFlexCellFormatter().setVerticalAlignment(0, 2,
                HasVerticalAlignment.ALIGN_TOP);
        ...
        tableProperties
                .setHTML(0, 0, "<b>" + Main.i18n("folder.name") + "</b>");
        tableProperties.setHTML(0, 1, folder.getName());
        tableProperties.setHTML(1, 0, "<b>" + Main.i18n("folder.parent")
                + "</b>");
        tableProperties.setHTML(1, 1, folder.getParentPath());
        tableProperties.setHTML(2, 0, "<b>" + Main.i18n("folder.created")
                + "</b>");
        final DateTimeFormat dtf = DateTimeFormat.getFormat(Main
                .i18n("general.date.pattern"));
        tableProperties.setHTML(2, 1, dtf.format(folder.getCreated()) + " "
                + Main.i18n("folder.by") + " " + folder.getAuthor());
        tableProperties.setHTML(3, 0, "<b>" + Main.i18n("document.subscribed")
                + "</b>");
        ...
        // Setting subscribers
        tableSubscribedUsers.setHTML(0, 0,
                "<b>" + Main.i18n("folder.subscribed.users") + "<b>");
        setRowWordWarp(0, 1, false, tableSubscribedUsers);
        ...
        final HorizontalPanel hKeyPanel = addKeywords(table,
                folder.getKeywords(), selectedKeyList); // Drawing keywords
    ...
    private void addMailRow(final GWTQueryResult gwtQueryResult,
        ...
        final Image gotoMail = new Image("img/icon/actions/goto_document.gif");
        gotoMail.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(final ClickEvent event) {
                CommonUI.openPath(
                        mailPath.substring(0, mailPath.lastIndexOf("/")),
                        mailPath);
            }
        ...
        // Writing detail
        rows++; // Next row line
        final FlexTable tableMail = new FlexTable();
        final FlexTable tableProperties = new FlexTable();
        final FlexTable tableSubscribedUsers = new FlexTable();
        tableMail.setWidget(0, 0, tableProperties);
        tableMail.setHTML(0, 1, "");
        tableMail.setWidget(0, 2, tableSubscribedUsers);
        tableMail.getFlexCellFormatter().setVerticalAlignment(0, 0,
                HasVerticalAlignment.ALIGN_TOP);
        tableMail.getFlexCellFormatter().setVerticalAlignment(0, 2,
                HasVerticalAlignment.ALIGN_TOP);
        ...
        tableProperties
                .setHTML(0, 0, "<b>" + Main.i18n("mail.folder") + "</b>");
        ...
        final DateTimeFormat dtf = DateTimeFormat.getFormat(Main
                .i18n("general.date.pattern"));
        tableProperties.setHTML(2, 1, dtf.format(mail.getCreated()) + " "
                + Main.i18n("mail.by") + " " + mail.getAuthor());
        tableProperties.setHTML(3, 0, "<b>" + Main.i18n("mail.mimetype")
                + "</b>");
        ...
        final HorizontalPanel hKeyPanel = addKeywords(table,
                mail.getKeywords(), selectedKeyList); // Drawing keywords
    ...
    public void setRefreshing() {
        final int left = Main.get().mainPanel.dashboard.keyMapDashboard.scrollTable
                .getAbsoluteLeft()
                + Main.get().mainPanel.dashboard.keyMapDashboard.scrollTable
                        .getOffsetWidth() / 2;
        final int top = Main.get().mainPanel.dashboard.keyMapDashboard.scrollTable
                .getAbsoluteTop()
                + Main.get().mainPanel.dashboard.keyMapDashboard.scrollTable
                        .getOffsetHeight() / 2;
     ...
     * @return
     */
    private HorizontalPanel addKeywords(final FlexTable table,
            final Set<String> keywords, final Collection<String> selectedKeyList) {
        int rows = table.getRowCount();
        // Writing keys
        rows++; // Next row line
        final HorizontalPanel hKeyPanel = new HorizontalPanel();
        table.setWidget(rows, 0, hKeyPanel);
        table.getFlexCellFormatter().setColSpan(rows, 0, 3);
        table.getCellFormatter().setHorizontalAlignment(rows, 0,
                HasHorizontalAlignment.ALIGN_RIGHT);
        table.getCellFormatter()
                .addStyleName(rows, 0, "okm-Table-BottomBorder");
        ...
        for (final String keyword : keywords) {
            ...
            if (!selectedKeyList.contains(keyword)) {
                ...
                final ImageHover add = new ImageHover(
                        "img/icon/actions/add_disabled.gif",
                        "img/icon/actions/add.gif");
