...
public class CategoriesPopup extends DialogBox {
    ...
    public CategoriesPopup() {
        ...
        table.setHTML(1, 0, "&nbsp;<b>" + Main.i18n("document.categories")
                + "</b>");
    ...
    public void reset() {
        ...
        if (!Main.get().mainPanel.desktop.browser.fileBrowser.isMassive()) {
            if (Main.get().mainPanel.desktop.browser.fileBrowser
                    .isDocumentSelected()) {
                assignedCategories = Main.get().mainPanel.desktop.browser.fileBrowser
                        .getDocument().getCategories();
            } else if (Main.get().mainPanel.desktop.browser.fileBrowser
                    .isFolderSelected()) {
                assignedCategories = Main.get().mainPanel.desktop.browser.fileBrowser
                        .getFolder().getCategories();
            } else if (Main.get().mainPanel.desktop.browser.fileBrowser
                    .isMailSelected()) {
                assignedCategories = Main.get().mainPanel.desktop.browser.fileBrowser
                        .getMail().getCategories();
            }
        }
    }
