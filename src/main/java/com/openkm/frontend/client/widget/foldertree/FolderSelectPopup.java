...
public class FolderSelectPopup extends DialogBox {
    ...
    public FolderSelectPopup() {
        ...
        vPanel.setCellHorizontalAlignment(scrollDirectoryPanel,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(status,
                HasHorizontalAlignment.ALIGN_CENTER);
     ...
     * Executes the action
     */
    public void executeAction(final String actualPath, final boolean refresh) {
        String fatherPath = "";
        ...
        case DOCUMENT:
            ...
            case ACTION_MOVE:
                ...
                if (!fatherPath.equals(actualPath)) {
                    setActionView();
                    documentService.move(((GWTDocument) node).getPath(),
                            actualPath, callbackMove);
                } else {
            ...
            case ACTION_COPY:
                ...
                if (!fatherPath.equals(actualPath)) {
                    setActionView();
                    documentService.copy(((GWTDocument) node).getPath(),
                            actualPath, callbackCopy);
                } else {
                    changeStatusOnError("fileupload.label.error.not.allowed.copy.same.folder");
                }
                break;
            ...
            case ACTION_CREATE_FROM_TEMPLATE:
                ...
                if (!fatherPath.equals(actualPath)) {
                    Main.get().templatePopup.reset((GWTDocument) node,
                            actualPath, true);
        ...
        case MAIL:
            ...
            case ACTION_MOVE:
                ...
                if (!fatherPath.equals(actualPath)) {
                    setActionView();
                    mailService.move(((GWTMail) node).getPath(), actualPath,
                            callbackMove);
                } else {
                    changeStatusOnError("fileupload.label.error.not.allowed.copy.same.folder");
                }
            ...
            case ACTION_COPY:
                ...
                if (!fatherPath.equals(actualPath)) {
                    setActionView();
                    mailService.copy(((GWTMail) node).getPath(), actualPath,
                            callbackCopy);
                } else {
                    changeStatusOnError("fileupload.label.error.not.allowed.copy.same.folder");
                }
        ...
        case FOLDER:
            ...
            case ACTION_MOVE:
                ...
                if (actualPath.indexOf(((GWTFolder) node).getPath()) == -1
                        && !((GWTFolder) node).getParentPath().equals(
                                actualPath)) {
                    setActionView();
                    folderService.move(((GWTFolder) node).getPath(),
                            actualPath, callbackMove);
                } else {
            ...
            case ACTION_COPY:
                ...
                if (!((GWTFolder) node).getPath().equals(actualPath)) {
                    setActionView();
                    folderService.copy(((GWTFolder) node).getPath(),
                            actualPath, callbackCopy);
                } else {
                    changeStatusOnError("fileupload.label.error.not.allowed.copy.same.folder");
                }
            ...
            case ACTION_RESTORE:
                setActionView();
                folderService.move(((GWTFolder) node).getPath(), actualPath,
                        callbackRestore);
                break;
            }
            break;
                ...
                case ACTION_MASSIVE_COPY:
                    ...
                    massiveService.copy(
                            Main.get().mainPanel.desktop.browser.fileBrowser
                                    .getAllSelectedPaths(), actualPath,
    ...
    public void show() {
        ...
        final int top = (Window.getClientHeight() - 440) / 2;
        ...
        public void onSuccess(final Object result) {
            ...
            case ENTRYPOINT_KEYBOARD:
                ...
                if (itemToBeRemoved != null) {
                    final TreeItem parentItem = itemToBeRemoved.getParentItem();
                    parentItem.removeItem(itemToBeRemoved);

                    if (parentItem.getChildCount() == 0) {
                        ((GWTFolder) parentItem.getUserObject())
                                .setHasChildren(false);
                    }

                    switch (originPanel) {
                    case UIDesktopConstants.NAVIGATOR_TAXONOMY:
                        Main.get().mainPanel.desktop.navigator.taxonomyTree
                                .evaluesFolderIcon(parentItem);
                    ...
                    case UIDesktopConstants.NAVIGATOR_PERSONAL:
                        Main.get().mainPanel.desktop.navigator.personalTree
                                .evaluesFolderIcon(parentItem);
                    ...
                    case UIDesktopConstants.NAVIGATOR_TEMPLATES:
                        Main.get().mainPanel.desktop.navigator.templateTree
                                .evaluesFolderIcon(parentItem);
                    ...
                    case UIDesktopConstants.NAVIGATOR_MAIL:
                        Main.get().mainPanel.desktop.navigator.mailTree
                                .evaluesFolderIcon(parentItem);
     ...
     */
    public void setToMove(final GWTDocument document) {
        type = DOCUMENT;
        node = document;
     ...
     */
    public void setToMove(final GWTMail mail) {
        type = MAIL;
        node = mail;
        action = ACTION_MOVE; // Sets the action
        actionButton.setText(Main.i18n("button.move"));
     ...
     * @param document The document object
     */
    public void setToCopy(final GWTDocument document) {
        type = DOCUMENT;
        node = document;
        action = ACTION_COPY;
        actionButton.setText(Main.i18n("button.copy"));
     ...
     * @param mail The mail object
     */
    public void setToCopy(final GWTMail mail) {
        type = MAIL;
        node = mail;
        action = ACTION_COPY;
        actionButton.setText(Main.i18n("button.copy"));
     ...
     * @param document The document object
     */
    public void setToCreateFromTemplate(final GWTDocument document) {
        type = DOCUMENT;
        node = document;
     ...
     */
    public void setToMove(final GWTFolder folder) {
        type = FOLDER;
        node = folder;
        action = ACTION_MOVE;
        actionButton.setText(Main.i18n("button.move"));
     ...
     * @param folder The folder object
     */
    public void setToCopy(final GWTFolder folder) {
        type = FOLDER;
        node = folder;
        action = ACTION_COPY;
        actionButton.setText(Main.i18n("button.copy"));
     ...
     * @param folder The folder object
     */
    public void setToMassiveCopy(final GWTFolder folder) {
        type = MASSIVE;
        node = folder;
     ...
     * @param folder The folder object
     */
    public void setToMassiveMove(final GWTFolder folder) {
        type = MASSIVE;
        node = folder;
     ...
     * @param document The document object
     */
    public void setToRestore(final GWTDocument document) {
        type = DOCUMENT;
        node = document;
     ...
     */
    public void setToRestore(final GWTMail mail) {
        type = MAIL;
        node = mail;
        action = ACTION_RESTORE;
        actionButton.setText(Main.i18n("button.restore"));
     ...
     * @param folder The folder object
     */
    public void setToRestore(final GWTFolder folder) {
        type = FOLDER;
        node = folder;
        action = ACTION_RESTORE;
        actionButton.setText(Main.i18n("button.restore"));
     ...
     */
    public boolean evaluateActionSecurity(final GWTFolder folder) {
     ...
     */
    public void setTreeItemToBeDeleted(final TreeItem item) {
     ...
     */
    public void setOriginPanel(final int originPanel) {
    ...
    private void setActionView() {
        ...
        status.setHTML(Util.imageItemHTML("img/indicator.gif")
                + Main.i18n(msgProperty));
        status.setStyleName("fancyfileupload-pending");
     ...
     */
    private void changeStatusOnError(final String msg) {
    ...
    public int getSelectedIndex() {
        return Integer.parseInt(contextListBox.getValue(contextListBox
                .getSelectedIndex()));
    }
