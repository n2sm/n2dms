...
public class FolderSelectPopup extends DialogBox {
    ...
    public void executeAction(String actualPath, boolean refresh) {
        ...
        case FOLDER:
            ...
            case ACTION_COPY:
                // Only copy when origin and destination path are not equals
                if (actualPath.indexOf(((GWTFolder) node).getPath()) == -1 && !((GWTFolder) node).getPath().equals(actualPath)) {
                    setActionView();
        ...
        case MASSIVE:
            ...
            if (!fatherPath.equals(actualPath)) {
                ...
                case ACTION_MASSIVE_COPY:
                    ...
                    massiveService.copy(Main.get().mainPanel.desktop.browser.fileBrowser.getAllSelectedPaths(), actualPath,
                ...
                case ACTION_MASSIVE_MOVE:
                    ...
                    massiveService.move(Main.get().mainPanel.desktop.browser.fileBrowser.getAllSelectedPaths(), actualPath,
                            new AsyncCallback<Object>() {
                                @Override
                                public void onSuccess(Object result) {
                                    action = ACTION_NONE; // Always resets initial value to ACTION_NONE
