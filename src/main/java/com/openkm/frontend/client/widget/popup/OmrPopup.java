...
public class OmrPopup extends DialogBox {
    private final OKMOmrServiceAsync omrService = (OKMOmrServiceAsync) GWT
            .create(OKMOmrService.class);
    ...
    public OmrPopup() {
        ...
        hPanel.setCellHorizontalAlignment(cancelButton,
                HasHorizontalAlignment.ALIGN_CENTER);
        hPanel.setCellHorizontalAlignment(executeButton,
                HasHorizontalAlignment.ALIGN_CENTER);
     ...
     * reset
     */
    public void reset(final GWTDocument doc) {
        this.doc = doc;
        executeButton.setEnabled(false);
        getOmr();
    }
    ...
    private void getOmr() {
        omrService.getAllOmr(new AsyncCallback<List<GWTOmr>>() {
            @Override
            public void onSuccess(final List<GWTOmr> result) {
                listBox.clear();
                listBox.addItem("", ""); // Adds empty value
                for (final GWTOmr omr : result) {
    ...
    private void process() {
        if (listBox.getSelectedIndex() > 0) {
            final long omId = Long.parseLong(listBox.getValue(listBox
                    .getSelectedIndex()));
            status.setFlagOmr();
            omrService.process(omId, doc.getUuid(),
