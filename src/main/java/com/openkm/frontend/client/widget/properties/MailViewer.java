...
public class MailViewer extends Composite {
    ...
    public MailViewer() {
        ...
        dataTable.getCellFormatter().setVerticalAlignment(0, 0,
                HasVerticalAlignment.ALIGN_TOP);
        dataTable.getCellFormatter().setVerticalAlignment(1, 0,
                HasVerticalAlignment.ALIGN_TOP);
        dataTable.getCellFormatter().setVerticalAlignment(2, 0,
                HasVerticalAlignment.ALIGN_TOP);
        dataTable.getCellFormatter().setVerticalAlignment(3, 0,
                HasVerticalAlignment.ALIGN_TOP);
        dataTable.getCellFormatter().setVerticalAlignment(1, 1,
                HasVerticalAlignment.ALIGN_TOP);
        dataTable.getCellFormatter().setVerticalAlignment(2, 1,
                HasVerticalAlignment.ALIGN_TOP);
        ...
        final HTML separatorLeft = new HTML("");
        final HTML separatorRight = new HTML("");
        content = new HTML("");
        ...
        attachmentText = new HTML("<b>" + Main.i18n("mail.attachment")
                + "</b>&nbsp;");
        attachmentsTable.setCellPadding(3);
        attachmentsTable.setCellSpacing(2);

        final VerticalPanel vRightPanel = new VerticalPanel();
    ...
    public void set(final GWTMail mail) {
        ...
        final Anchor hFrom = new Anchor();
        final String mailFrom = mail.getFrom().contains("<") ? mail.getFrom()
                .substring(mail.getFrom().indexOf("<") + 1,
                        mail.getFrom().indexOf(">")) : mail.getFrom();
        ...
        for (int i = 0; i < mail.getTo().length; i++) {
            ...
            final String mailTo = mail.getTo()[i].contains("<") ? mail.getTo()[i]
                    .substring(mail.getTo()[i].indexOf("<") + 1,
        ...
        for (final GWTDocument attach : mail.getAttachments()) {
            ...
            final int row = attachmentsTable.getRowCount();
            attachmentsTable.setHTML(row, 0,
                    Util.mimeImageHTML(attach.getMimeType()));
            attachmentsTable.setWidget(row, 1, hLink);
            attachmentsTable.setHTML(row, 2,
                    Util.formatSize(attach.getActualVersion().getSize()));
            attachmentsTable.getCellFormatter().setHorizontalAlignment(row, 2,
     ...
     * @return The attach
     */
    public GWTDocument getAttach(final int selectedRow) {
        // Attachements are writem from second row
        if (attachmentsTable.getSelectedRow() >= 0
                && attachmentsList.containsKey(new Integer(selectedRow))) {
        ...
        if (attachmentsTable.getSelectedRow() >= 0) {
            if (getAttach(attachmentsTable.getSelectedRow()) != null) {
                Util.downloadFileByUUID(
                        getAttach(attachmentsTable.getSelectedRow()).getUuid(),
                        "");
            }
        }
    }
    ...
    public void copyAttachment() {
        ...
        if (attachmentsTable.getSelectedRow() >= 0) {
            Main.get().activeFolderTree.folderSelectPopup
                    .setEntryPoint(FolderSelectPopup.ENTRYPOINT_MAIL_ATTACH);
            Main.get().activeFolderTree.folderSelectPopup
                    .setToCopy(getAttach(attachmentsTable.getSelectedRow()));
    ...
    public void langRefresh() {
        ...
        dataTable.setHTML(2, 0, "<b>" + Main.i18n("mail.to") + "</b>");
        dataTable.setHTML(3, 0, "<b>" + Main.i18n("mail.subject") + "</b>");
        attachmentText.setHTML("<b>" + Main.i18n("mail.attachment")
                + "</b>&nbsp;");
