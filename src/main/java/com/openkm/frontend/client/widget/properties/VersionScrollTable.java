...
public class VersionScrollTable extends Composite implements ClickHandler {
    ...
    public VersionScrollTable() {
        ...
        headerTable.getCellFormatter().setHorizontalAlignment(0, 5,
                HasHorizontalAlignment.ALIGN_CENTER);
        headerTable.getCellFormatter().setVerticalAlignment(0, 5,
    ...
    public void langRefresh() {
        ...
        if (!buttonView.isEmpty()) {
            for (final Button button : buttonView) {
                button.setHTML(Main.i18n("button.view"));
            }
        }

        if (!buttonRestore.isEmpty()) {
            for (final Button button : buttonRestore) {
     ...
     */
    private void addRow(final GWTVersion version) {
        ...
        if (version.isActual() && visibleButtons) {
            ...
            final Button restoreButton = new Button(
                    Main.i18n("button.restore"), new ClickHandler() {
                        @Override
                        public void onClick(final ClickEvent event) {
                            final List<String> versions = Main.get().mainPanel.desktop.browser.tabMultiple.tabDocument.version.versions;
                            final String ver = versions.get(rows);
                            Main.get().confirmPopup
                                    .setConfirm(ConfirmPopup.CONFIRM_RESTORE_HISTORY_DOCUMENT);
        ...
        final Button viewButton = new Button(Main.i18n("button.view"),
                new ClickHandler() {
                    @Override
                    public void onClick(final ClickEvent event) {
                        final List<String> versions = Main.get().mainPanel.desktop.browser.tabMultiple.tabDocument.version.versions;
        ...
        viewButton.setVisible(Main.get().workspaceUserProperties.getWorkspace()
    ...
    final AsyncCallback<List<GWTVersion>> callbackGetVersionHistory = new AsyncCallback<List<GWTVersion>>() {
        @Override
        public void onSuccess(final List<GWTVersion> result) {
            removeAllRows();
            ...
            for (final GWTVersion version : result) {
                addRow(version);
            }
    ...
    public void getVersionHistory() {
        if (doc != null) {
            Main.get().mainPanel.desktop.browser.tabMultiple.status
                    .setVersionHistory();
            documentService.getVersionHistory(doc.getPath(),
                    callbackGetVersionHistory);
        }
    }
     ...
     */
    public void restoreVersion(final String versionId) {
        if (doc != null) {
            Main.get().mainPanel.desktop.browser.tabMultiple.status
                    .setRestoreVersion();
            documentService.restoreVersion(doc.getPath(), versionId,
                    callbackRestoreVersion);
        }
    }
    ...
    public void purgeVersionHistory() {
        if (doc != null) {
            Main.get().mainPanel.desktop.browser.tabMultiple.status
                    .setPurgeVersionHistory();
            documentService.purgeVersionHistory(doc.getPath(),
                    callbackPurgeVersionHistory);
        }
    }
    ...
    public void onClick(final ClickEvent event) {
        Main.get().confirmPopup
                .setConfirm(ConfirmPopup.CONFIRM_PURGE_VERSION_HISTORY_DOCUMENT);
        Main.get().confirmPopup.show();
    }
