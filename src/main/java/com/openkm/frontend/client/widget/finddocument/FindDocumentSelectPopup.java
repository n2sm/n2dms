...
public class FindDocumentSelectPopup extends DialogBox {
    ...
    public FindDocumentSelectPopup() {
        ...
        actionButton = new Button(Main.i18n("search.result.menu.go.document"),
                new ClickHandler() {
                    @Override
                    public void onClick(final ClickEvent event) {
                        final String docPath = documentTable.getText(
                                selectedRow, 1);
            ...
            public void onKeyUp(final KeyUpEvent event) {
                if (keyword.getText().length() >= 3) {
                    ...
                    case UIDesktopConstants.NAVIGATOR_TAXONOMY:
                        gwtParams.setPath(Main.get().taxonomyRootFolder
                                .getPath());
                        break;
                    case UIDesktopConstants.NAVIGATOR_TEMPLATES:
                        gwtParams.setPath(Main.get().templatesRootFolder
                                .getPath());
                        break;
                    case UIDesktopConstants.NAVIGATOR_PERSONAL:
                        gwtParams.setPath(Main.get().personalRootFolder
                                .getPath());
                        break;
                    ...
                    gwtParams
                            .setProperties(new HashMap<String, GWTPropertyParams>());
                    find(gwtParams);
                } else {
                    removeAllRows();
                }
        ...
        documentTable.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(final ClickEvent event) {
                markSelectedRow(documentTable.getCellForEvent(event)
                        .getRowIndex());
        ...
        documentTable.addDoubleClickHandler(new DoubleClickHandler() {
            ...
            public void onDoubleClick(final DoubleClickEvent event) {
                final String docPath = documentTable.getText(selectedRow, 1);
                final String path = docPath.substring(0,
                        docPath.lastIndexOf("/"));
                CommonUI.openPath(path, docPath);
                hide();
        ...
        vPanel.setCellHorizontalAlignment(keyword,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellVerticalAlignment(keyword,
                HasVerticalAlignment.ALIGN_MIDDLE);
        vPanel.setCellHorizontalAlignment(scrollDocumentPanel,
                HasHorizontalAlignment.ALIGN_CENTER);
    ...
    public void show() {
        initButtons();
        final int left = (Window.getClientWidth() - 700) / 2;
    ...
    private void styleRow(final int row, final boolean selected) {
        if (row >= 0) {
            if (selected) {
                documentTable.getRowFormatter().addStyleName(row,
                        "okm-Table-SelectedRow");
            } else {
                documentTable.getRowFormatter().removeStyleName(row,
                        "okm-Table-SelectedRow");
            }
        }
    }
    ...
    final AsyncCallback<GWTResultSet> callbackFind = new AsyncCallback<GWTResultSet>() {
        ...
        public void onSuccess(final GWTResultSet result) {
            final GWTResultSet resultSet = result;
            removeAllRows();

            for (final GWTQueryResult gwtQueryResult : resultSet.getResults()) {
                if (gwtQueryResult.getDocument() != null) {
                    final GWTDocument doc = gwtQueryResult.getDocument();
                    final int rows = documentTable.getRowCount();
                    documentTable.setHTML(rows, 0,
                    ...
                    documentTable.getCellFormatter().setHorizontalAlignment(
                            rows, 0, HasHorizontalAlignment.ALIGN_CENTER);
                }
            }
     ...
     * @param params
     */
    private void find(final GWTQueryParams params) {
