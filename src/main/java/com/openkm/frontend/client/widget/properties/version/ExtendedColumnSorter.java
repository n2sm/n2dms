...
public class ExtendedColumnSorter extends ColumnSorter {
    ...
    public void sort(int column, boolean ascending) {
        ...
        Map<Integer, GWTVersion> data =
                new HashMap<Integer, GWTVersion>(Main.get().mainPanel.desktop.browser.tabMultiple.tabDocument.version.data);

        List<GWTObjectToOrder> elementToOrder = new ArrayList<GWTObjectToOrder>(); // List with column data,  and actual position

        if (column >= 0 && column <= 6 && column != UIDesktopConstants.DOCUMENT_HISTORY_COLUMN_BUTTON_SHOW
    ...
    private void applySort(List<GWTObjectToOrder> elementToOrder) {
        ...
        for (GWTObjectToOrder orderedColumn : elementToOrder) {
            Main.get().mainPanel.desktop.browser.tabMultiple.tabDocument.version
                    .addRow(data.get(Integer.parseInt(orderedColumn.getDataId())));
        }
    }
}