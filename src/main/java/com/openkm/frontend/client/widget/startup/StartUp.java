...
public class StartUp implements HasWidgetHandlerExtension, HasWidgetEvent {
    ...
    public void init() {
        generalService.getEnabledExtensions(new AsyncCallback<List<String>>() {
            ...
            public void onSuccess(final List<String> result) {
                ...
                ExtensionManager.start(Customization
                        .getExtensionWidgets(result));
     ...
     * startKeepAlive
     */
    public void startKeepAlive(final double scheduleTime) {
        // KeepAlieve thread
        keepAlive = new Timer() {
            @Override
            public void run() {
        ...
        if (docPath != null && !docPath.equals("")) {
            documentService.isValid(docPath, new AsyncCallback<Boolean>() {
                ...
                public void onSuccess(final Boolean result) {
                    ...
                    Main.get().startUp
                            .nextStatus(StartUp.STARTUP_LOADING_OPEN_PATH);
                }
            ...
            folderService.isValid(fldPath, new AsyncCallback<Boolean>() {
                ...
                public void onSuccess(final Boolean result) {
                    if (result.booleanValue()) {
                        final boolean isRoot = Util.isRoot(fldPath);
                        if (!isRoot) {
                            ...
                            folderService.getProperties(fldPath,
                                    new AsyncCallback<GWTFolder>() {
                                        @Override
                                        public void onSuccess(
                                                final GWTFolder result) {
                                            folderService
                                                    .getProperties(
                                                            fldPath,
                                                            new AsyncCallback<GWTFolder>() {
     ...
     */
    public void nextStatus(final int status) {
        if (enabled) {
            ...
            if (this.status < status) {
                ...
                case STARTUP_STARTING:
                    Main.get().startUpPopup.addStatus(
                            Main.i18n("startup.starting.loading"),
                            STARTUP_STARTING);
                ...
                case STARTUP_GET_USER_VALUES:
                    Main.get().startUpPopup.addStatus(
                            Main.i18n("startup.loading.user.values"),
                ...
                case STARTUP_GET_TAXONOMY_ROOT:
                    if (Main.get().workspaceUserProperties.getWorkspace()
                            .isStackTaxonomy()) {
                        Main.get().startUpPopup.addStatus(
                                Main.i18n("startup.taxonomy"),
                                STARTUP_GET_TAXONOMY_ROOT);
                        getRoot();
                    } else {
                ...
                case STARTUP_GET_TEMPLATE_ROOT:
                    if (Main.get().workspaceUserProperties.getWorkspace()
                        ...
                        Main.get().startUpPopup.addStatus(
                                Main.i18n("startup.template"),
                                STARTUP_GET_TEMPLATE_ROOT);
                        getTemplate();
                    } else {
                        Main.get().startUpPopup.jumpActual();
                        nextStatus(STARTUP_GET_PERSONAL);
                ...
                case STARTUP_GET_PERSONAL:
                    if (Main.get().workspaceUserProperties.getWorkspace()
                        ...
                        Main.get().startUpPopup.addStatus(
                                Main.i18n("startup.personal"),
                                STARTUP_GET_PERSONAL);
                        getPersonal();
                    } else {
                        Main.get().startUpPopup.jumpActual();
                        nextStatus(STARTUP_GET_MAIL);
                ...
                case STARTUP_GET_MAIL:
                    if (Main.get().workspaceUserProperties.getWorkspace()
                            .isStackMailVisible()) {
                        Main.get().startUpPopup.addStatus(
                                Main.i18n("startup.mail"), STARTUP_GET_MAIL);
                        getMail();
                    } else {
                        Main.get().startUpPopup.jumpActual();
                        nextStatus(STARTUP_GET_TRASH);
                ...
                case STARTUP_GET_TRASH:
                    if (Main.get().workspaceUserProperties.getWorkspace()
                            .isStackTrashVisible()) {
                        Main.get().startUpPopup.addStatus(
                                Main.i18n("startup.trash"), STARTUP_GET_TRASH);
                        getTrash();
                    } else {
                        Main.get().startUpPopup.jumpActual();
                        nextStatus(STARTUP_GET_USER_HOME);
                ...
                case STARTUP_GET_USER_HOME:
                    ...
                    Main.get().mainPanel.desktop.browser.fileBrowser.table
                            .fillWidth(); // Sets de columns size
                    ...
                    Main.get().mainPanel.desktop.browser.tabMultiple.tabMail
                            .init();
                    Main.get().startUpPopup.addStatus(
                            Main.i18n("startup.user.home"),
                            STARTUP_GET_USER_HOME);
                    getUserHome();
                    break;

                case STARTUP_GET_BOOKMARKS:
                    Main.get().startUpPopup.addStatus(
                            Main.i18n("startup.bookmarks"),
                            STARTUP_GET_BOOKMARKS);
                ...
                case STARTUP_INIT_TREE_NODES:
                    Main.get().startUpPopup.addStatus(
                            Main.i18n("startup.init.tree.nodes"),
                    ...
                    if (Main.get().workspaceUserProperties.getWorkspace()
                            .isStackCategoriesVisible()) {
                        Main.get().mainPanel.desktop.navigator.categoriesTree
                                .init();
                    }
                    if (Main.get().workspaceUserProperties.getWorkspace()
                            .isStackThesaurusVisible()) {
                        Main.get().mainPanel.desktop.navigator.thesaurusTree
                                .init();
                    }
                    if (Main.get().workspaceUserProperties.getWorkspace()
                            .isStackTemplatesVisible()) {
                    ...
                    Main.get().startUp
                            .nextStatus(StartUp.STARTUP_LOADING_HISTORY_SEARCH);
                    break;

                case STARTUP_LOADING_HISTORY_SEARCH:
                    Main.get().startUpPopup.addStatus(
                            Main.i18n("startup.loading.history.search"),
                ...
                case STARTUP_LOADING_TAXONOMY_EVAL_PARAMS:
                    Main.get().startUpPopup.addStatus(
                            Main.i18n("startup.loading.taxonomy.eval.params"),
                ...
                case STARTUP_LOADING_OPEN_PATH:
                    Main.get().startUpPopup.addStatus(
                            Main.i18n("startup.loading.taxonomy.open.path"),
    ...
    public String getStatusMsg(final int status) {
        String msg = "";
    ...
    @Override
    public void addWidgetHandlerExtension(
            final WidgetHandlerExtension handlerExtension) {
        widgetHandlerExtensionList.add(handlerExtension);
    }

    @Override
    public void fireEvent(final WidgetEventConstant event) {
        for (final WidgetHandlerExtension handlerExtension : widgetHandlerExtensionList) {
