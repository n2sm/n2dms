...
public class NotifyPopup extends DialogBox {
    ...
    public NotifyPopup() {
        ...
        commentTXT = new HTML("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                + Main.i18n("fileupload.label.notify.comment"));
        ...
        vPanel.setCellHorizontalAlignment(errorNotify,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(messageScroll,
                HasHorizontalAlignment.ALIGN_CENTER);
        vPanel.setCellHorizontalAlignment(notifyPanel,
                HasHorizontalAlignment.ALIGN_CENTER);
     ...
     * @param type
     */
    public void executeSendDocument(final int type) {
        if (Main.get().mainPanel.desktop.browser.fileBrowser
                .isDocumentSelected()) {
            reset(type);
            doc = Main.get().mainPanel.desktop.browser.fileBrowser
                    .getDocument();
            super.center();
    ...
    private void sendLinkNotification() {
        switch (type) {
        case NOTIFY_WITH_LINK:
            notifyService.notify(doc.getPath(), users, roles,
                    message.getText(), false, callbackNotify);
            break;
        case NOTIFY_WITH_ATTACHMENT:
            notifyService.notify(doc.getPath(), users, roles,
                    message.getText(), true, callbackNotify);
            break;
        }
    }
     ...
     * Reste values
     */
    private void reset(final int type) {
        this.type = type;
        switch (type) {
        case NOTIFY_WITH_LINK:
            setText(Main.i18n("notify.label"));
