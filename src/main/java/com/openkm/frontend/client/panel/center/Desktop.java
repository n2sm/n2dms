...
public class Desktop extends Composite {
    ...
    public Desktop() {
        ...
        horizontalSplitPanel.getSplitPanel().setSplitPosition(
                "" + PANEL_LEFT_WIDTH);
    ...
    private void resizePanels() {
        final int total = horizontalSplitPanel.getOffsetWidth();

        String valWidth = DOM.getStyleAttribute(DOM.getChild(DOM.getChild(
                horizontalSplitPanel.getSplitPanel().getElement(), 0), 0),
                "width");
        if (valWidth.contains("px")) {
            valWidth = valWidth.substring(0, valWidth.indexOf("px"));
        }
        left = Integer.parseInt(valWidth);

        String valLeft = DOM.getStyleAttribute(DOM.getChild(DOM.getChild(
                horizontalSplitPanel.getSplitPanel().getElement(), 0), 2),
                "left");
        if (valLeft.contains("px")) {
            valLeft = valLeft.substring(0, valLeft.indexOf("px"));
        }
    ...
    private void timeControl() {
        if (TimeHelper.hasElapsedEnoughtTime(TIME_HELPER_KEY,
            ...
            if (!finalResizeInProgess) {
                ...
                final int total = horizontalSplitPanel.getOffsetWidth();
                String value = DOM.getStyleAttribute(DOM.getChild(DOM.getChild(
                        horizontalSplitPanel.getSplitPanel().getElement(), 0),
                        0), "width");
                if (value.contains("px")) {
                    value = value.substring(0, value.indexOf("px"));
                }
                left = Integer.parseInt(value);
                value = DOM.getStyleAttribute(DOM.getChild(DOM.getChild(
                        horizontalSplitPanel.getSplitPanel().getElement(), 0),
                        2), "left");
                if (value.contains("px")) {
                    value = value.substring(0, value.indexOf("px"));
