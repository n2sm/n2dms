...
public class ImageLogoServlet extends HttpServlet {
    ...
    public void doGet(final HttpServletRequest request,
            ...
            ServletException {
        final String img = request.getPathInfo();

        try {
            if (img != null && img.length() > 1) {
                final String logo = logos.get(img);

                if (logo != null) {
                    final InputStream is = getServletContext().getResource(
                            "/img/" + logo).openStream();
                    WebUtils.sendFile(request, response, logo,
                            MimeTypeConfig.MIME_GIF, true, is);
                } else {
    ...
    private void sendError(final HttpServletRequest request,
        ...
        final InputStream is = getServletContext()
                .getResource("/img/error.png").openStream();
