...
public class DatabaseQueryServlet extends BaseServlet {
        ...
        try {
            if (ServletFileUpload.isMultipartContent(request)) {
                ...
                if (action.equals("edit")) {
                    ...
                    String hql =
                            "update DatabaseMetadataValue dmv set dmv." + column + "='" + value + "' where dmv.table='" + vtable
                                    + "' and dmv.id=" + id;
                    log.info("HQL: {}", hql);
                    session = HibernateUtil.getSessionFactory().openSession();
        ...
        if (hql.toUpperCase().startsWith("SELECT") || hql.toUpperCase().startsWith("FROM")) {
            ...
            if (vtables == null) {
                ...
                for (String vtable : vtables) {
                    String query =
                            "select dmt.virtualColumn, dmt.realColumn from DatabaseMetadataType dmt where dmt.table='" + vtable + "'";
    ...
    private List<String> listTables(Session session) {
        ...
        final String[] tablePatterns =
                new String[] { "JBPM_%", "OKM_%", "DEFAULT_%", "VERSION_%", "jbpm_%", "okm_%", "default_%", "version_%" };
