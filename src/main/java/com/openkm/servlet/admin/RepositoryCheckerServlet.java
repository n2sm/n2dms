...
public class RepositoryCheckerServlet extends BaseServlet {
    ...
    public void doGet(final HttpServletRequest request,
        ...
        final String repoPath = WebUtils.getString(request, "repoPath", "/"
                + Repository.ROOT);
        final boolean versions = WebUtils.getBoolean(request, "versions");
        ...
        try {
            if (!repoPath.equals("")) {
                ...
                final ContentInfo cInfo = OKMFolder.getInstance()
                        .getContentInfo(null, repoPath);
                ...
                if (Config.REPOSITORY_NATIVE) {
                    stats = DbRepositoryChecker.checkDocuments(null, repoPath,
                            versions, out,
                            new HTMLInfoDecorator((int) cInfo.getDocuments()));
                } else {
                    stats = JcrRepositoryChecker.checkDocuments(null, repoPath,
                            versions, out,
                            new HTMLInfoDecorator((int) cInfo.getDocuments()));
                }
                ...
                out.println("<div class=\"ok\">Versions: " + versions
                        + "</div>");
                out.println("<br/>");
                out.println("<b>Documents:</b> " + stats.getDocuments()
                        + "<br/>");
                out.println("<b>Folders:</b> " + stats.getFolders() + "<br/>");
                out.println("<b>Size:</b> "
                        + FormatUtil.formatSize(stats.getSize()) + "<br/>");
                out.println("<b>Time:</b> "
                        + FormatUtil.formatSeconds(end - begin) + "<br/>");

                // Activity log
                UserActivity.log(
                        request.getRemoteUser(),
                        "ADMIN_REPOSITORY_CHECKER",
                        null,
                        null,
                        "Documents: " + stats.getDocuments() + ", Folders: "
                                + stats.getFolders() + ", Size: "
                                + FormatUtil.formatSize(stats.getSize())
                                + ", Time: "
