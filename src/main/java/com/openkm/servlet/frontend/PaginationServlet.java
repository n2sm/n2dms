...
public class PaginationServlet extends OKMRemoteServiceServlet implements OKMPaginationService {
    ...
    @Override
    public GWTPaginated getChildrenPaginated(String fldPath, boolean extraColumns, int offset, int limit, int order, boolean reverse,
            boolean folders, boolean documents, boolean mails, String selectedRowId, Map<String, GWTFilter> mapFilter) throws OKMException {
                    ...
                    if (found) {
                        if (actual < offset + limit) {
                            if (obj.getObj() instanceof NodeFolder) {
                                ...
                                GWTFolder gWTFolder = (extraColumns) ? GWTUtil.copy(fld, workspace) : GWTUtil.copy(fld, null);
                                ...
                                GWTDocument gWTDoc = (extraColumns) ? GWTUtil.copy(doc, workspace) : GWTUtil.copy(doc, null);
                                gwtCol.add(gWTDoc);
                            } else if (obj.getObj() instanceof NodeMail) {
                                Mail mail = BaseMailModule.getProperties(user, (NodeMail) obj.getObj());
                                GWTMail gWTMail = (extraColumns) ? GWTUtil.copy(mail, workspace) : GWTUtil.copy(mail, null);
                                gwtCol.add(gWTMail);
                            }
                        } else {
                            break;
