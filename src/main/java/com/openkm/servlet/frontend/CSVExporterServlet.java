    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            CSVStrategy strategyFormat =
                    new CSVStrategy(Config.CSV_FORMAT_DELIMITER.toCharArray()[0], Config.CSV_FORMAT_QUOTE_CHARACTER.toCharArray()[0],
            ...
            CSVWriter<String[]> csvWriter =
                    new CSVWriterBuilder<String[]>(out).strategy(strategyFormat).entryConverter(new DefaultCSVEntryConverter()).build();
            csvWriter.writeAll(csvValues);
            csvWriter.flush();
        } catch (ParseException e) {
            log.error(e.getMessage(), e);
            throw new ServletException(new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMCSVExporterService, ErrorCode.CAUSE_Parse),
                    e.getMessage()));
        } catch (RepositoryException e) {
            log.error(e.getMessage(), e);
            throw new ServletException(new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMCSVExporterService, ErrorCode.CAUSE_Repository),
                    e.getMessage()));
        } catch (DatabaseException e) {
            log.error(e.getMessage(), e);
            throw new ServletException(new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMCSVExporterService, ErrorCode.CAUSE_Database),
                    e.getMessage()));
        } catch (AccessDeniedException e) {
            log.error(e.getMessage(), e);
            throw new ServletException(new OKMException(
                    ErrorCode.get(ErrorCode.ORIGIN_OKMCSVExporterService, ErrorCode.CAUSE_AccessDenied), e.getMessage()));
        } catch (PathNotFoundException e) {
            log.error(e.getMessage(), e);
            throw new ServletException(new OKMException(
                    ErrorCode.get(ErrorCode.ORIGIN_OKMCSVExporterService, ErrorCode.CAUSE_PathNotFound), e.getMessage()));
            ...
            log.error(e.getMessage(), e);
            throw new ServletException(new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMCSVExporterService, ErrorCode.CAUSE_NoSuchGroup),
                    e.getMessage()));
        }
    }
}