    ...
    public boolean isGranted(ItemId id, int permissions) throws ItemNotFoundException, RepositoryException {
        if (DEBUG)
            log.debug("isGranted("
                    ...
                    + id
                    + ", "
                    + (permissions == AccessManager.READ ? "READ" : (permissions == AccessManager.WRITE ? "WRITE"
                            : (permissions == AccessManager.REMOVE ? "REMOVE" : "NONE"))) + ")");
                ...
                if (nPerms == null) {
                    ...
                    if (node == null) {
                        ...
                        if (node.isNodeType(Document.CONTENT_TYPE)) {
                            ...
                            if (DEBUG)
                                log.debug(subject.getPrincipals() + " Node is NOTE_LIST_TYPE");
                            node = node.getParent();
                            if (DEBUG)
                                log.debug(subject.getPrincipals() + " Real -> " + node.getPath());
                        } else if (node.isNodeType(Note.TYPE)) {
                            if (DEBUG)
                                log.debug(subject.getPrincipals() + " Node is NOTE_TYPE");
        ...
        if (DEBUG)
            log.debug(subject.getPrincipals()
                    + " isGranted "
                    + (permissions == AccessManager.READ ? "READ" : (permissions == AccessManager.WRITE ? "WRITE"
                            : (permissions == AccessManager.REMOVE ? "REMOVE" : "NONE"))) + ": " + access);
        if (DEBUG)
            log.debug("-------------------------------------");
        return access;
    }
