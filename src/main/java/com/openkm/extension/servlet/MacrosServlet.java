 ...
 * 
 */
public class MacrosServlet extends OKMRemoteServiceServlet implements
        OKMMacrosService {
    ...
    public List<GWTMacros> getActions() throws OKMException {
        ...
        final List<GWTMacros> actionList = new ArrayList<GWTMacros>();

        try {
            final String actions[] = ConfigDAO.getText("macros.actions", "")
                    .split("\r\n");

            for (final String action : actions) {
                final String act[] = action.split(",");
                ...
                if (act.length > 1) {
                    ...
                    final GWTMacros fastAction = new GWTMacros();
            ...
            throw new OKMException(ErrorCode.get(
                    ErrorCode.ORIGIN_OKMFastActionService,
    ...
    @Override
    public void executeAction(final GWTMacros action, final String path)
            throws OKMException {
        updateSessionManager();
        ...
        try {
            ...
            final String rightPath = path.substring(orgPath.length() + 1); // Right path

            if (rightPath.indexOf("/") >= 0) {
                final String extraFldPath = rightPath.substring(0,
                        rightPath.lastIndexOf("/"));
                final String destFoldersToCreate[] = extraFldPath.split("/");
                for (int i = 0; i < destFoldersToCreate.length; i++) { // Put all destination files path
                    if (i == 0) {
                        destFoldersToCreate[i] = dstPath + "/"
                                + destFoldersToCreate[i];
                    } else {
                        destFoldersToCreate[i] = destFoldersToCreate[i - 1]
                                + "/" + destFoldersToCreate[i];
                    }
                }
            ...
            // Move to destination
            final String dstPathToMove = dstPath + "/" + rightPath;
            final String dstParentFolder = dstPathToMove.substring(0,
                    dstPathToMove.lastIndexOf("/"));
            ...
            throw new OKMException(
                    ErrorCode.get(ErrorCode.ORIGIN_OKMFastActionService,
     ...
     * moveFolderContents
     */
    private void moveFolderContents(final String fldPath,
            final String dstPathToMove) throws PathNotFoundException,
            RepositoryException, DatabaseException, ItemExistsException,
            AccessDeniedException, LockException, ExtensionException,
            AutomationException {
        // Move documents
        for (final Document doc : OKMDocument.getInstance().getChildren(null,
                fldPath)) {
            OKMDocument.getInstance().move(null, doc.getPath(), dstPathToMove);
        }

        // Move Mails
        for (final Mail mail : OKMMail.getInstance().getChildren(null, fldPath)) {
            OKMMail.getInstance().move(null, mail.getPath(), dstPathToMove);
        }

        // Move subfolders
        for (final Folder fld : OKMFolder.getInstance().getChildren(null,
                fldPath)) {
            final String fldToMove = dstPathToMove
                    + "/"
                    + fld.getPath().substring(
                            fld.getPath().lastIndexOf("/") + 1);
            // Control case if folder exists
            if (!OKMRepository.getInstance().hasNode(null, fldToMove)) {
                OKMFolder.getInstance()
                        .move(null, fld.getPath(), dstPathToMove);
            } else {
     ...
     * createFolders
     */
    private void createFolders(final String orgPath, final String dstPath,
            final String folders[]) throws OKMException, RepositoryException,
            DatabaseException, PathNotFoundException, ItemExistsException,
        ...
        for (final String folder : folders) {
            if (!OKMRepository.getInstance().hasNode(null, folder)) {
                ...
                // Change security
                final String originPath = folder.replaceFirst(dstPath, orgPath);
                final Map<String, Integer> dstRolesMap = OKMAuth.getInstance()
                        .getGrantedRoles(null, folder);
                final Map<String, Integer> dstUsersMap = OKMAuth.getInstance()
                        .getGrantedUsers(null, folder);
                final Map<String, Integer> orgRolesMap = OKMAuth.getInstance()
                        .getGrantedRoles(null, originPath);
                final Map<String, Integer> orgUsersMap = OKMAuth.getInstance()
                        .getGrantedUsers(null, originPath);

                // Add full grants to actual remote user
                final String remoteUser = getThreadLocalRequest()
                        .getRemoteUser();
                final int allGrants = Permission.ALL_GRANTS;

                OKMAuth.getInstance().grantUser(null, folder, remoteUser,
                        allGrants, false);

                // Remove all grants except remote user
                for (final String role : dstRolesMap.keySet()) {
                    OKMAuth.getInstance().revokeRole(null, folder, role,
                            allGrants, false);
                }

                for (final String user : dstUsersMap.keySet()) {
                    if (!remoteUser.equals(user)) {
                        OKMAuth.getInstance().revokeUser(null, folder, user,
                                allGrants, false);
                    }
                }

                // Setting privileges except remote user
                for (final String role : orgRolesMap.keySet()) {
                    OKMAuth.getInstance().grantRole(null, folder, role,
                            orgRolesMap.get(role).intValue(), false);
                }

                for (final String user : orgUsersMap.keySet()) {
                    if (!remoteUser.equals(user)) {
                        OKMAuth.getInstance().grantUser(null, folder, user,
                                orgUsersMap.get(user).intValue(), false);
                    }
                }

                // Setting user privileges if exists, otherside revokes all
                if (orgUsersMap.containsKey(remoteUser)) {
                    final int permission = orgUsersMap.get(remoteUser)
                            .intValue();

                    if ((permission & Permission.READ) != Permission.READ) {
                        OKMAuth.getInstance().revokeUser(null, folder,
                                remoteUser, Permission.READ, false);
                    }

                    if ((permission & Permission.DELETE) != Permission.DELETE) {
                        OKMAuth.getInstance().revokeUser(null, folder,
                                remoteUser, Permission.DELETE, false);
                    }

                    if ((permission & Permission.WRITE) != Permission.WRITE) {
                        OKMAuth.getInstance().revokeUser(null, folder,
                                remoteUser, Permission.WRITE, false);
                    }

                    if ((permission & Permission.SECURITY) != Permission.SECURITY) {
                        OKMAuth.getInstance().revokeUser(null, folder,
                                remoteUser, Permission.SECURITY, false);
                    }
                } else {
                    OKMAuth.getInstance().revokeUser(null, folder, remoteUser,
                            allGrants, false);
                }
            }
        }
