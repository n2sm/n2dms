...
public class OpenMeetingsServlet extends OKMRemoteServiceServlet implements OKMOpenMeetingsService {
    ...
    public String loginUser() throws OKMException {
        ...
        try {
            ...
            throw new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMOpenMeetingsService, ErrorCode.CAUSE_OpenMeetings), e.getMessage());
        }
    }
    ...
    @Override
    public String getPublicRoomURL(String SID, long roomId, boolean moderator, boolean showAudioVideoTest, String lang) throws OKMException {
        ...
        try {
            ...
            roomHash =
                    UserService.setUserObjectAndGenerateRoomHashByURL(SID, userId, firstName, lastName, profilePictureUrl, email, userId,
                            externalUserType, roomId, becomeModeratorAsInt, showAudioVideoTestAsInt);

            url =
                    OpenMeetingsUtils.getOpenMeetingServerURL() + "/?scopeRoomId=" + String.valueOf(roomId) + "&secureHash=" + roomHash
                            + "&language=" + OpenMeetingsUtils.getOpenMeetingsLang(lang) + "&lzproxied=solo";
        } catch (PrincipalAdapterException e) {
            log.error(e.getMessage(), e);
            throw new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMOpenMeetingsService, ErrorCode.CAUSE_PrincipalAdapter), e.getMessage());
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            throw new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMOpenMeetingsService, ErrorCode.CAUSE_OpenMeetings), e.getMessage());
        }
    ...
    @Override
    public void createNewRoom(String SID, String name, long roomType, long numberOfPartizipants, boolean isPublic, boolean appointment,
            boolean moderated, boolean allowUserQuestions, boolean audioOnly, boolean waitForRecording, boolean allowRecording,
            boolean topBar) throws OKMException {
        log.debug("createNewRoom({},{},{},{},{},{},{},{},{},{},{},{})", new Object[] { SID, name, roomType, numberOfPartizipants, isPublic,
                appointment, moderated, allowUserQuestions, audioOnly, waitForRecording, allowRecording, topBar });
        updateSessionManager();
        try {
            String externalRoomType = getRoomType(roomType);
            long roomId =
                    RoomService.addRoomWithModerationExternalTypeAndTopBarOption(SID, name, roomType, "", numberOfPartizipants, isPublic,
                            appointment, false, moderated, externalRoomType, allowUserQuestions, audioOnly, waitForRecording,
                            allowRecording, topBar);
            ...
            if (roomType == GWTRoom.TYPE_RESTRICTED || roomType == GWTRoom.TYPE_INTERVIEW) {
                ...
                UserService.setUserObjectAndGenerateRoomHashByURL(SID, userId, firstName, "", "", email, userId, externalUserType, roomId,
    ...
    public List<GWTRoom> getPrivateConferenceUserRooms(String SID) throws OKMException {
        ...
        try {
            ...
            for (Room room : RoomService.getRoomsWithCurrentUsersByListAndType(SID, 0, Integer.MAX_VALUE, orderBy, true, externalRoomType)) {
    ...
    @Override
    public void sendInvitation(String SID, long roomId, String users, String roles, String subject, String message, String lang)
            throws OKMException {
        try {
            ...
            for (String userId : userNames) {
                ...
                UserService.setUserObjectAndGenerateRoomHashByURL(SID, userId, firstName, "", "", email, userId, "", roomId,
                        becomeModeratorAsInt, showAudioVideoTestAsInt);

                // Invitation
                RoomService.sendInvitationHash(SID, userId, message, baseUrl, email, subject, roomId, conferencedomain,
                        isPasswordProtected, invitationpass, valid, validFromDate, validFromTime, validToDate, validToTime, languageId,
                        sendMail);
            }
        } catch (PrincipalAdapterException e) {
            log.error(e.getMessage(), e);
