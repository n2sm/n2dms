...
public class SubMenuZoho {
    ...
    public SubMenuZoho() {
        // All menu items
        zohoWriterMenu =
                new MenuItemExtension("img/icon/actions/zoho_writer.png", GeneralComunicator.i18nExtension("zoho.writer"), zohoWriter);
        zohoSheetMenu = new MenuItemExtension("img/icon/actions/zoho_sheet.png", GeneralComunicator.i18nExtension("zoho.sheet"), zohoSheet);
        ...
        // Principal menu
        zohoMenu = new MenuItemExtension("img/icon/actions/zoho.png", GeneralComunicator.i18nExtension("zoho.menu"), subMenu);
    ...
    public void langRefresh() {
        zohoWriterMenu
                .setHTML(UtilComunicator.menuHTML("img/icon/actions/zoho_writer.png", GeneralComunicator.i18nExtension("zoho.writer")));
        zohoSheetMenu.setHTML(UtilComunicator.menuHTML("img/icon/actions/zoho_sheet.png", GeneralComunicator.i18nExtension("zoho.sheet")));
        zohoMenu.setHTML(UtilComunicator.menuHTML("img/icon/actions/zoho.png", GeneralComunicator.i18nExtension("zoho.menu")));
    }
    ...
    public void evaluateMenus() {
        ...
        if (FileBrowserComunicator.isDocumentSelected()) {
            ...
            if ((doc.getMimeType().equals("application/msword") && doc.getName().endsWith("doc"))
                    ...
                    || doc.getMimeType().equals("text/html") || doc.getMimeType().equals("application/pdf")
                    || (doc.getMimeType().equals("application/vnd.oasis.opendocument.text") && doc.getName().endsWith("odt"))
                    || doc.getMimeType().equals("application/rtf") || doc.getMimeType().equals("text/plain")) {
                ...
                if (((doc.getPermissions() & GWTPermission.WRITE) == GWTPermission.WRITE)
                    ...
                    || (doc.getMimeType().equals("application/vnd.oasis.opendocument.spreadsheet") && doc.getName().endsWith("ods"))
                    || doc.getMimeType().equals("text/csv")) {
                if (((doc.getPermissions() & GWTPermission.WRITE) == GWTPermission.WRITE)
                        && ((folder.getPermissions() & GWTPermission.WRITE) == GWTPermission.WRITE)) {
                    if (doc.isCheckedOut() || doc.isLocked()) {
                        if (doc.getLockInfo().getOwner().equals(GeneralComunicator.getUser())
